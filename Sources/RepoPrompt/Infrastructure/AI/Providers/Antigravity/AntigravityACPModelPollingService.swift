...
protocol AntigravityACPModelDiscoveryClient: Sendable {
...
actor AntigravityACPControllerModelDiscoveryClient: AntigravityACPModelDiscoveryClient {
    ...
    init(
        providerFactory: @escaping ProviderFactory = { agent, modelString in
            if agent == .antigravity {
                return AntigravityACPAgentProvider()
            }
    ...
    func discoverModels(workspacePath: String?) async throws -> ACPDiscoveredSessionModels? {
        ...
        guard let provider = try await providerFactory(.antigravity, nil) else { return nil }
        let support = try await provider.support(for: request)
        guard support == .supported else {
            throw AIProviderError.invalidConfiguration(
                detail: support.reason ?? "Antigravity ACP is not available."
            )
        }
        ...
        do {
            ...
            let snapshot = AgentACPModelRegistry.shared.currentSnapshot(for: .antigravity)
    ...
    private static func stableDiscoveryWorkspacePath() -> String {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("RepoPromptAntigravityACPDiscovery", isDirectory: true)
        try? FileManager.default.createDirectory(at: url, withIntermediateDirectories: true, attributes: nil)
        return url.standardizedFileURL.path
    }
}

// SEARCH-HELPER: Antigravity ACP model polling, dynamic discovery, subscribe, registry refresh
/// Centralized polling service for Antigravity ACP dynamic model options. Mirrors the Cursor
...
actor AntigravityACPModelPollingService {
    static let shared = AntigravityACPModelPollingService(
        client: AntigravityACPControllerModelDiscoveryClient()
    )
    ...
    private let client: any AntigravityACPModelDiscoveryClient
    ...
    init(
        client: any AntigravityACPModelDiscoveryClient,
        ...
        let task = Task<Bool, Never> { [weak self, workspacePath] in
            ...
            do {
                ...
                if AgentRuntimeProviderService.enableDebugLogging {
                    print("[AntigravityACPModelPolling] refresh failed: \(error.localizedDescription)")
                }
                return false
            }
        }
    ...
    private func publishLiveReadinessWithoutModels() {
        ...
        let models = latest?.models
            ?? AgentACPModelRegistry.shared.resolvedSnapshot(for: .antigravity)
    ...
    private func applyRefreshResult(_ discovered: ACPDiscoveredSessionModels) {
        ...
        _ = AgentACPModelRegistry.shared.updateDiscoveredModels(discovered, for: .antigravity)
        guard let normalized = AgentACPModelRegistry.shared.resolvedSnapshot(for: .antigravity) else { return }
    ...
    private func registrySnapshotAfterWarmingStore() async -> Snapshot? {
        guard let models = await AgentACPModelRegistry.shared.resolvedSnapshotAfterWarmingStandardStore(for: .antigravity) else {
            return nil
        }
