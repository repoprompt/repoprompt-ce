    ...
    private func createMessages(for aiMessage: AIMessage) -> [CompletionParams.Message] {
        ...
        for (index, entry) in conversation.enumerated() {
            ...
            if role == .user {
                ...
                let content: CompletionParams.Message.Content =
                    (index == lastUserIndex && !aiMessage.transientImages.isEmpty)
                        ? .parts(Self.imageParts(text: userContent, images: aiMessage.transientImages))
                        : .text(userContent)
        ...
        if lastUserIndex == nil, !aiMessage.transientImages.isEmpty {
            results.append(CompletionParams.Message(
                role: .user,
                content: .parts(Self.imageParts(
                    text: additionsForFinalUserMessage,
