...
enum PathCharPolicy {
    @inline(__always)
    static func isAllowedASCIIByte(_ b: UInt8) -> Bool {
        switch b {
        case 0x30 ... 0x39, // 0-9
             0x41 ... 0x5A, // A-Z
             0x61 ... 0x7A, // a-z
    ...
    @inline(__always)
    static func toLowerASCII(_ b: UInt8) -> UInt8 {
    ...
    @inline(__always)
    static func isZeroWidthOrFormat(_ sc: UnicodeScalar) -> Bool {
        switch sc.value {
    ...
    @inline(__always)
    static func emitFolded(_ sc: UnicodeScalar, into view: inout String.UnicodeScalarView) {
        let v = sc.value
    ...
    static func foldHomoglyphsIfNeeded(_ s: String) -> String {
