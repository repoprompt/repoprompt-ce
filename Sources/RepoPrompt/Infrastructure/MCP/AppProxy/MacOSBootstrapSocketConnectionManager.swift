...
actor BootstrapSocketConnectionManager: MCPServerConnection {
    ...
    func start(approvalHandler: @escaping (MCP.Client.Info) async -> Bool) async throws {
        guard !isClosing else { throw MCPError.connectionClosed }
                    ...
                    if !hasInFlight {
                        ...
                        Task { [parentManager] in
                            await parentManager.terminateConnection(
                                connectionID,
                                reason: .idleTimeout,
                                message: "Connection idle for \(Int(idle))s"
                            )
                        }
                        break
                    }
    ...
    private func shutdown() async {
        ...
        isClosing = true
        let healthTask = healthMonitoringTask
        let watcherTask = closeWatchTask
        healthTask?.cancel()
        watcherTask?.cancel()

        let task = Task { [self, transport, server, healthTask, watcherTask] in
            await transport.disconnect()
            await server.stop()
    ...
    #if DEBUG
        func debugTransportCleanupSnapshot() async -> UnixSocketMCPTransportCleanupSnapshot {
            await transport.debugCleanupSnapshot()
        }
    #endif
