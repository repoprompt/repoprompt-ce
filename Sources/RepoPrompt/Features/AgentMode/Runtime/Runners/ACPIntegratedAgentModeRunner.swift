...
final class ACPIntegratedAgentModeRunner {
    ...
    private func applyExplicitSelectedModelIfNeeded(
        ...
        guard runRequest.agentKind == .openCode || runRequest.agentKind == .cursor || runRequest.agentKind == .grokBuild || runRequest.agentKind == .antigravity else { return }
        ...
        if runRequest.agentKind == .grokBuild || runRequest.agentKind == .antigravity,
           AgentACPModelRegistry.shared.resolvedSnapshot(for: runRequest.agentKind.acpProviderID!)?.contains(rawModel: model) != true
        {
            ...
            throw AIProviderError.invalidConfiguration(
                detail: "ACP model `\(model)` is not in the discovered model set. Refresh Antigravity models and retry."
            )
        }
