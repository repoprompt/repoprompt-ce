...
/// Standalone helper for file-tree generation and selection presentation.
enum CodeMapExtractor {
    ...
    /// Legacy FileViewModel tree renderers have no renderable presentation identity.
    /// Modern tree presentation supplies marker IDs through its immutable operation snapshot.
    private static func collectCodeMapIDs(from _: [FolderViewModel]) -> Set<UUID> {
        []
    }
