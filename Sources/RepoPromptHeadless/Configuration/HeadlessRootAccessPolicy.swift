...
final class HeadlessRootAccessPolicy: WorkspaceAccessPolicy {
    private let allowedRoots: [HeadlessAllowedRoot]
    ...
    func allowsWorkspaceRoot(_ url: URL) -> Bool {
        let resolvedPath = Self.resolvedPath(for: url)
        return allowedRoots.contains { root in
            Self.path(resolvedPath, isContainedInOrEqualTo: root.resolvedPath)
        }
    }
