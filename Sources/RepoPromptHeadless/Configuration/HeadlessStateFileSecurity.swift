...
enum HeadlessStateFileSecurity {
    ...
    private static func openPrivateDirectory(at url: URL, stateRoot: URL) throws -> Int32 {
        let standardizedRoot = stateRoot.standardizedFileURL
        let standardizedURL = url.standardizedFileURL
        let rootPath = standardizedRoot.path
        let targetPath = standardizedURL.path
        guard targetPath == rootPath || targetPath.hasPrefix(rootPath + "/") else {
            throw HeadlessCommandError("Private state path escapes its state root: \(targetPath)", exitCode: 2)
        }

        var descriptor = try openPrivateDirectory(at: standardizedRoot)
    ...
    private static func openPrivateDirectory(at url: URL) throws -> Int32 {
        let standardizedURL = url.standardizedFileURL
        guard standardizedURL.path.hasPrefix("/"), !standardizedURL.lastPathComponent.isEmpty else {
            throw HeadlessCommandError("Private state directory must be an absolute non-root path: \(url.path)", exitCode: 2)
        }
        let parentDescriptor = try openDirectoryPathCreatingMissing(standardizedURL.deletingLastPathComponent().path)
        defer { Darwin.close(parentDescriptor) }
        ...
        if mkdirResult != 0, errno != EEXIST {
            throw posixError(operation: "create private state directory", path: standardizedURL.path, errorNumber: errno)
        }
        let descriptor = try openLeaf(
            name,
            relativeTo: parentDescriptor,
    ...
    private static func openDirectoryPathCreatingMissing(_ path: String) throws -> Int32 {
        var ancestor = URL(fileURLWithPath: path, isDirectory: true).standardizedFileURL
        var missingComponents: [String] = []
        while !pathExistsWithoutFollowingLeaf(ancestor.path) {
            let component = ancestor.lastPathComponent
            guard !component.isEmpty else {
                throw HeadlessCommandError("Unable to find an existing ancestor for private state path: \(path)", exitCode: 2)
            }
            missingComponents.insert(component, at: 0)
            ancestor.deleteLastPathComponent()
        }
        guard let canonicalAncestor = canonicalExistingPath(ancestor.path) else {
            throw HeadlessCommandError("Unable to resolve private state ancestor: \(ancestor.path)", exitCode: 2)
        }

        var descriptor = try openCanonicalDirectoryPath(canonicalAncestor)
        do {
    ...
    private static func openCanonicalDirectoryPath(_ path: String) throws -> Int32 {
    ...
    private static func pathExistsWithoutFollowingLeaf(_ path: String) -> Bool {
        var status = stat()
