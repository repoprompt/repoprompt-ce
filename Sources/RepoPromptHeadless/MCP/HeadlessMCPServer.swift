...
actor HeadlessMCPServer {
    ...
    private func handleRequest(method: String, id: Any, object: [String: Any]) -> HeadlessRPCSubmission {
        ...
        default:
            ...
            return executeReadyRequest(method: method, id: id, object: object)
        }
    }

    private func executeReadyRequest(method: String, id: Any, object: [String: Any]) -> HeadlessRPCSubmission {
        switch method {
        case "ping":
        ...
        case "tools/call":
            ...
            return startToolRequest(id: id, name: name, arguments: arguments)
        case "shutdown":
    ...
    private func startToolRequest(id: Any, name: String, arguments: HeadlessJSONObject) -> HeadlessRPCSubmission {
        guard let key = RequestKey(id: id) else {
            return .completed(requestError(hasID: true, id: id, code: -32600, message: "JSON-RPC request id must be a string, number, or null."))
        }
        guard activeRequests[key] == nil else {
            return .completed(requestError(hasID: true, id: id, code: -32600, message: "A request with the same JSON-RPC id is already active."))
        }
        ...
        let task = Task { [weak self, registry, toolCallOverride] in
            ...
            do {
                ...
                let result: HeadlessJSONObject = if let toolCallOverride {
                    ...
                    await registry.call(name: name, arguments: arguments)
                }
                try Task.checkCancellation()
            ...
            await self?.requestCompleted(key)
            return action
        }
