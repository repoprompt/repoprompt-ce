...
package struct CodeMapCaptureIndex {
    /// All captures sorted by location
    package let all: [NamedRange]

    /// Captures grouped by name, each list sorted by location
    package let byName: [String: [NamedRange]]

    /// Creates an index from an array of captures
    package init(_ captures: [NamedRange]) {
    ...
    package func captures(named name: String) -> [NamedRange] {
        byName[name] ?? []
    }

    /// Returns the first capture with the given name that is fully contained within the parent range
    package func firstCapture(named name: String, containedIn parent: NSRange) -> NamedRange? {
    ...
    package func captures(named name: String, containedIn parent: NSRange) -> [NamedRange] {
    ...
    package func allCaptures(containedIn parent: NSRange) -> [NamedRange] {
        var results: [NamedRange] = []
    ...
    package func smallestCapture(named name: String, containing target: NSRange) -> NamedRange? {
        guard let candidates = byName[name] else { return nil }
    ...
    package func smallestCapture(namedAny names: [String], containing target: NSRange) -> NamedRange? {
