...
package actor CodeScanActor {
    ...
    package init(
        cacheRoot: URL,
        maxConcurrentScans: Int = CodeScanActor.defaultMaxConcurrentScans
    ) {
        cacheManager = CodeMapCacheManager(baseDirectory: cacheRoot)
    ...
    /// -------------------------------------
    package enum ScanBatchPurpose {
        case initialRootLoad
        case adhoc
    }

    package struct ScanRequest {
        let fileID: UUID
    ...
    package struct ScanResult: @unchecked Sendable {
    ...
    #if DEBUG
        package struct CodemapMemoryCounters {
            package let fileAPIEntryCount: Int
            package let latestFileModDateCount: Int
            package let trackedRootCount: Int
            package let trackedFileIDCount: Int
            package let rootKeyByFileIDCount: Int
            package let rootCacheRootCount: Int
            package let rootCacheFileEntryCount: Int
            package let dirtyRootCount: Int
            package let rootCacheLoadTaskCount: Int
            package let rebuildLookupRootCount: Int
            package let rebuildLookupFileEntryCount: Int
            package let queuedCount: Int
            package let activeScanCount: Int
            package let outstandingScanCount: Int
            package let totalScheduledCount: Int
            package let cacheProcessingCount: Int
            package let resultBatchBufferCount: Int
            package let resultBatchBufferFileAPICount: Int
            package let actorRetainedFileAPILikeEntryCount: Int

            package init(
                fileAPIEntryCount: Int,
                latestFileModDateCount: Int,
                trackedRootCount: Int,
                trackedFileIDCount: Int,
                rootKeyByFileIDCount: Int,
                rootCacheRootCount: Int,
                rootCacheFileEntryCount: Int,
                dirtyRootCount: Int,
                rootCacheLoadTaskCount: Int,
                rebuildLookupRootCount: Int,
                rebuildLookupFileEntryCount: Int,
                queuedCount: Int,
                activeScanCount: Int,
                outstandingScanCount: Int,
                totalScheduledCount: Int,
                cacheProcessingCount: Int,
                resultBatchBufferCount: Int,
                resultBatchBufferFileAPICount: Int,
                actorRetainedFileAPILikeEntryCount: Int
            ) {
                self.fileAPIEntryCount = fileAPIEntryCount
                self.latestFileModDateCount = latestFileModDateCount
                self.trackedRootCount = trackedRootCount
                self.trackedFileIDCount = trackedFileIDCount
                self.rootKeyByFileIDCount = rootKeyByFileIDCount
                self.rootCacheRootCount = rootCacheRootCount
                self.rootCacheFileEntryCount = rootCacheFileEntryCount
                self.dirtyRootCount = dirtyRootCount
                self.rootCacheLoadTaskCount = rootCacheLoadTaskCount
                self.rebuildLookupRootCount = rebuildLookupRootCount
                self.rebuildLookupFileEntryCount = rebuildLookupFileEntryCount
                self.queuedCount = queuedCount
                self.activeScanCount = activeScanCount
                self.outstandingScanCount = outstandingScanCount
                self.totalScheduledCount = totalScheduledCount
                self.cacheProcessingCount = cacheProcessingCount
                self.resultBatchBufferCount = resultBatchBufferCount
                self.resultBatchBufferFileAPICount = resultBatchBufferFileAPICount
                self.actorRetainedFileAPILikeEntryCount = actorRetainedFileAPILikeEntryCount
            }
        }
    #endif
    ...
    /// -------------------------------------------------------
    package nonisolated func subscribeToScanResults() -> AsyncStream<[ScanResult]> {
        let id = UUID()
    ...
    /// -------------------------------------------------------
    package nonisolated func subscribeToProgress() -> AsyncStream<(Int, Int)> {
        let id = UUID()
        return AsyncStream { continuation in
    ...
    /// -------------------------------------------------------
    package func cancelAndUnloadScans(forRootFolder rootFolderPath: String) async {
    ...
    /// -------------------------------------------------------
    package func cancelAndUnloadScans(forRootFolders rootFolderPaths: [String]) async {
    ...
    /// -------------------------------------------------------
    package func requestScan(_ request: ScanRequest) async {
    ...
    package func requestScans(
        ...
        if purpose == .initialRootLoad {
            #if DEBUG
                let cacheRebuildStartMS = CodeMapRuntimeDiagnostics.start()
            #endif
            initialRootLookupByRoot = await prepareCacheRebuild(forRoots: rootsForRebuild)
            #if DEBUG
                CodeMapRuntimeDiagnostics.cacheRebuild(
        ...
        #if DEBUG
            let cacheCheckStartMS = purpose == .initialRootLoad ? CodeMapRuntimeDiagnostics.start() : nil
        #endif
        ...
        #if DEBUG
            if purpose == .initialRootLoad {
                CodeMapRuntimeDiagnostics.cacheCheck(
        ...
        if purpose == .initialRootLoad {
            #if DEBUG
                let pruneStartMS = CodeMapRuntimeDiagnostics.start()
            #endif
            pruneInitialRootCaches(
                forRoots: rootsForRebuild,
                keepingRelativePathsByRoot: initialRootRelativePathsByRoot,
            ...
            #if DEBUG
                CodeMapRuntimeDiagnostics.prune(
                    rootCount: rootsForRebuild.count,
        ...
        if !queueableRequests.isEmpty {
            #if DEBUG
                let enqueueStartMS = purpose == .initialRootLoad ? CodeMapRuntimeDiagnostics.start() : nil
            #endif
            ...
            #if DEBUG
                if purpose == .initialRootLoad {
                    CodeMapRuntimeDiagnostics.enqueue(
                        queueableRequests: queueableRequests.count,
    ...
    /// -------------------------------------------------------
    package nonisolated func requestScan(_ request: ScanRequest) {
        Task { await self.requestScan(request) }
    }

    package nonisolated func requestScans(
        _ requests: [ScanRequest],
        purpose: ScanBatchPurpose = .adhoc,
        rootFolderPaths: [String] = [],
        purgeCachesOnEmptyInitialRequests: Bool = false
    ...
    package func clearAllCaches(rootFolders: [String]) {
    ...
    package func purgeStaleRootCaches(keepingRootPaths: [String]) {
    ...
    #if DEBUG
        package func codemapMemoryCounters() -> CodemapMemoryCounters {
