...
package struct ReferencedTypesAccumulator {
    package let language: LanguageType
    ...
    package init(language: LanguageType, stats: CodeMapPerfStats? = nil, perfOptions: CodeMapPerfOptions = .disabled) {
        self.language = language
        self.stats = stats
        self.perfOptions = perfOptions
    }

    package mutating func insert(rawType: String?) {
    ...
    package mutating func insertMany(rawTypes: [String]) {
        for rawType in rawTypes {
            insert(rawType: rawType)
        }
    }

    package func finalizeSorted() -> [String] {
        Array(types).sorted()
    }

    package var rawInsertions: Int {
        rawInsertCount
    }
}
