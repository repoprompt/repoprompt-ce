...
package struct CodeMapPCRE2Match {
    private let captures: [String?]

    package init(subject: String, match: PCRE2Match) {
    ...
    package func capture(_ index: Int) -> String? {
        guard index >= 0, index < captures.count else { return nil }
        return captures[index]
    }

    package func trimmedCapture(_ index: Int) -> String? {
        capture(index)?.trimmingCharacters(in: .whitespaces)
    }
}

package struct CodeMapPCRE2Pattern {
    ...
    package func firstMatch(in text: String) -> CodeMapPCRE2Match? {
        guard let match = try? regex.firstMatch(in: text) else { return nil }
        return CodeMapPCRE2Match(subject: text, match: match)
    }

    package func firstCapture(_ index: Int = 1, in text: String) -> String? {
        firstMatch(in: text)?.capture(index)
    }

    package func trimmedCapture(_ index: Int = 1, in text: String) -> String? {
    ...
    package func wholeMatch(in text: String) -> Bool {
        guard let match = try? regex.firstMatch(in: text) else { return false }
        return match.byteRange == 0 ..< text.utf8.count
    }

    package func replacingMatches(in text: String, with replacement: String = "") -> String {
