...
package enum CodeMapPerfRuntime {
    ...
    private static let markerRootLock = NSLock()
    private static var configuredBenchmarkMarkerRoot = FileManager.default.temporaryDirectory

    package static func installBenchmarkMarkerRoot(_ root: URL) {
        markerRootLock.lock()
        configuredBenchmarkMarkerRoot = root.standardizedFileURL
        markerRootLock.unlock()
    }

    static var benchmarkMarkerURL: URL {
        markerRootLock.lock()
        defer { markerRootLock.unlock() }
        return configuredBenchmarkMarkerRoot.appendingPathComponent("run-codemap-benchmarks", isDirectory: false)
    }
