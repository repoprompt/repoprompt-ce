...
package enum FileTreeSnapshotRenderer {
    package static func generateFileTree(using snapshot: FileTreeSelectionSnapshot) -> String {
        ...
        func buildOnce(
            ...
            for (index, root) in effectiveRoots.enumerated() {
                ...
                if let currentRemaining = remaining {
                    let consumedTokens = text.isEmpty ? 0 : max(1, snapshotEstimateTokens(for: text))
        ...
        for attempt in attempts {
            ...
            if snapshotEstimateTokens(for: text) <= snapshotAutoTokenBudget {
...
@inline(__always)
private func snapshotEstimateTokens(for text: String) -> Int {
