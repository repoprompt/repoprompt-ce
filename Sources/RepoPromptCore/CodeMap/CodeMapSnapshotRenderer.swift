...
package enum CodeMapSnapshotRenderer {
    package static func generateFileTree(using snapshot: FileTreeSelectionSnapshot) -> String {
        ...
        func buildOnce(
            ...
            for (index, root) in effectiveRoots.enumerated() {
                ...
                if let currentRemaining = remaining {
                    let consumedTokens = text.isEmpty ? 0 : max(1, TokenEstimator.estimateTokens(for: text))
        ...
        for attempt in attempts {
            ...
            if TokenEstimator.estimateTokens(for: text) <= snapshotAutoTokenBudget {
