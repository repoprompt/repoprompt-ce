...
package enum TopLevelScanner {
    package struct TrackDelimiters: OptionSet {
        package let rawValue: Int
        package static let angle = TrackDelimiters(rawValue: 1 << 0) // < >
        package static let paren = TrackDelimiters(rawValue: 1 << 1) // ( )
        package static let brace = TrackDelimiters(rawValue: 1 << 2) // { }
        package static let square = TrackDelimiters(rawValue: 1 << 3) // [ ]
        package static let all: TrackDelimiters = [.angle, .paren, .brace, .square]
    ...
    package static func firstTopLevelIndex(
        of char: Character,
        in input: String,
        track: TrackDelimiters = .all
    ) -> String.Index? {
    ...
    package static func firstTopLevelRange(
        of needle: String,
        in input: String,
        track: TrackDelimiters = .all
    ) -> Range<String.Index>? {
    ...
    package static func containsTopLevel(
        _ needle: String,
        in input: String,
        track: TrackDelimiters = .all
    ) -> Bool {
