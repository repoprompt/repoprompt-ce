...
package actor FileSystemService {
    ...
    package func publishFileSystemDeltas(
        ...
        let publication = FileSystemDeltaPublication(
            ...
            WorkspaceRuntimePerf.$currentFileSystemPublicationCorrelation.withValue(publicationCorrelation) {
                changePublisher.send(publication)
            }
        #else
            changePublisher.send(publication)
    ...
    nonisolated let directoryAccess: any WorkspaceDirectoryAccessing
    nonisolated let contentSnapshotReader: any FileContentSnapshotReading
    nonisolated let contentDecoder: any FileContentDecoding
    nonisolated let runtimeDiagnostics: any RuntimeDiagnosticsSink
    ...
    let changePublisher = OrderedEventBroadcaster<FileSystemDeltaPublication>()
    ...
    init(
        ...
        watcherIngressMailbox = FileSystemWatcherIngressMailbox(
            maxQueuedRawEntries: runtimeDependencies.configuration.maxPendingWatcherEntries
        )
        watcherEarlyFilter = FileSystemWatcherEarlyFilter(rootPath: path)
        fileSystemWatcher = runtimeDependencies.watcherFactory.makeWatcher(path: path)
        fileMutationBackend = runtimeDependencies.mutationBackend
        directoryAccess = runtimeDependencies.directoryAccess
        contentSnapshotReader = runtimeDependencies.contentSnapshotReader
        contentDecoder = runtimeDependencies.contentDecoder
        runtimeDiagnostics = runtimeDependencies.diagnostics
        WorkspaceRuntimePerf.installProcessSink(runtimeDependencies.diagnostics)
        ignoreRulesManager = IgnoreRulesManager(
            globalIgnoreDefaults: runtimeDependencies.configuration.globalIgnoreDefaults,
            contentSnapshotReader: runtimeDependencies.contentSnapshotReader
        )
        nextFSEventStreamStartEventID = runtimeDependencies.currentWatchEventID()

        // Configure parallelism caps based on available cores
        let cores = ProcessInfo.processInfo.activeProcessorCount
        maxParallelScansPerActor = runtimeDependencies.configuration.maxParallelScans ?? max(2, min(4, cores / 2))
        maxFoldersPerBatch = runtimeDependencies.configuration.maxFoldersPerBatch
        maxRecoveryScanAttempts = runtimeDependencies.configuration.maxRecoveryScanAttempts
        recoveryScanRetryBaseNanoseconds = runtimeDependencies.configuration.recoveryScanRetryBaseNanoseconds
        ...
        package init(
            ...
            watcherIngressMailbox = FileSystemWatcherIngressMailbox(
                maxQueuedRawEntries: maxPendingWatcherIngressEntriesOverride
                    ?? runtimeDependencies.configuration.maxPendingWatcherEntries
            )
            watcherEarlyFilter = FileSystemWatcherEarlyFilter(rootPath: path)
            fileSystemWatcher = runtimeDependencies.watcherFactory.makeWatcher(path: path)
            fileMutationBackend = runtimeDependencies.mutationBackend
