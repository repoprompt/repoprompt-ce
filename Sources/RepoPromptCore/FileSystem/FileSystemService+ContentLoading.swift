...
private func detectEncodingFull(
    _ data: Data,
    decoder: any FileContentDecoding
) -> String.Encoding {
    String.Encoding(
        rawValue: decoder.detectEncodingRawValue(in: data)
...
private struct ContentReadRequest {
    ...
    let snapshotReader: any FileContentSnapshotReading
    let decoder: any FileContentDecoding
    #if DEBUG
...
package struct FileContentPrefix {
...
package actor ContentReadAsyncLimiter {
    #if DEBUG
        package struct Snapshot: Equatable {
            package let capacity: Int
            package let maxQueuedWaiterCount: Int
            package let activePermitCount: Int
            package let queuedWaiterCount: Int
            package let ownerLaneCount: Int
            package let cancellationCount: Int
            package let grantCount: Int
            package let overloadCount: Int
            package let interactiveGrantCount: Int
            package let normalGrantCount: Int
            package let bulkGrantCount: Int
            package let backgroundPermitLimit: Int
            package let activeBackgroundPermitCount: Int
    ...
    private func acquire(
        ...
        guard waiterStates.count < maxQueuedWaiterCount else {
            ...
            WorkspaceRuntimePerf.lifecycleEvent(
                WorkspaceRuntimePerf.Lifecycle.FileSystem.contentReadWorkerOverloaded,
...
extension FileSystemService {
    ...
    #if DEBUG
        package nonisolated static var contentReadWorkerLimitForTesting: Int {
            contentReadWorkerLimit
        }

        package nonisolated static func contentReadWorkerLimiterSnapshotForTesting() async -> ContentReadAsyncLimiter.Snapshot {
    ...
    func loadValidatedContent(
        ...
        let result = try await performMeasuredContentReadOffActor(
            request,
            lifecycleCorrelation: WorkspaceRuntimePerf.currentLifecycleCorrelation,
    ...
    private func commitContentReadResultIfCurrent(_ result: ContentReadResult, cacheKey: String) {
        ...
        guard (try? FileContentFingerprintReader.fingerprint(
            atPath: result.absolutePath,
            reader: contentSnapshotReader
    ...
    private nonisolated static func readContentPrefixFromDisk(
        ...
        if isProbablyBinary(probe, decoder: request.decoder) {
            return nil
        }
        ...
        let encoding: String.Encoding = detectBOMEncoding(in: prefixData) ?? detectEncodingFull(prefixData, decoder: request.decoder)
    ...
    private nonisolated static func readContentFromDisk(
        ...
        do {
            ...
            if requirePostReadValidation {
                let postReadFingerprint = try FileContentFingerprintReader.fingerprint(
                    atPath: request.absolutePath,
                    reader: request.snapshotReader
                )
    ...
    private nonisolated static func readAutomaticContent(
        ...
        if validated.fileSize < 2_000_000 {
            ...
            let detected = try decodeSmallFileData(data, decoder: request.decoder)
    ...
    private nonisolated static func readStreamedContent(
        ...
        let detector = request.decoder.makeEncodingDetectionSession()

        try await runContentReadChunkHook(request)
        let initialData = try handle.read(upToCount: request.chunkSize) ?? Data()
        try Task.checkCancellation()
        if !skipProbe, isProbablyBinary(initialData, decoder: request.decoder) {
            return try validateOpenContentHandle(
                handle,
                snapshotReader: request.snapshotReader,
                validated: validated,
    ...
    private nonisolated static func validateContentFileForReading(_ request: ContentReadRequest) throws -> ValidatedContentFile {
        ...
        let fingerprint = try FileContentFingerprintReader.fingerprint(
            atPath: standardizedAbsolutePath,
            reader: request.snapshotReader
        )
    ...
    private nonisolated static func openValidatedContentHandle(
        ...
        let handle = try FileContentFingerprintReader.openReadOnlyFileHandle(
            atPath: request.absolutePath,
            reader: request.snapshotReader
        )
        do {
    ...
    private nonisolated static func validateOpenContentHandle(
        _ handle: FileHandle,
        snapshotReader: any FileContentSnapshotReading,
        validated: ValidatedContentFile,
        result: ContentReadResult,
        required: Bool
    ...
    private nonisolated static func decodeSmallFileData(
        _ data: Data,
        decoder: any FileContentDecoding
    ) throws -> DetectedText {
    ...
    private nonisolated static func runContentReadChunkHook(_ request: ContentReadRequest) async throws {
        ...
        private func loadContentSerialForTesting(_ request: ContentReadRequest) async throws -> String? {
            ...
            if !skipProbe, let handle = try? FileHandle(forReadingFrom: url) {
                let probe = try handle.read(upToCount: 8192) ?? Data()
                try? handle.close()
                if Self.isProbablyBinary(probe, decoder: request.decoder) { return nil }
            }
        ...
        private func loadEntireFileContentOptimizedSerialForTesting(_ request: ContentReadRequest) async throws -> String? {
            ...
            let detector = request.decoder.makeEncodingDetectionSession()
            let initialData = try handle.read(upToCount: request.chunkSize) ?? Data()
            if !skipProbe, Self.isProbablyBinary(initialData, decoder: request.decoder) { return nil }
            fullData.append(initialData)
            _ = detector.analyzeNextChunk(initialData)
            try Task.checkCancellation()
    ...
    func detectEncodingForInitialChunk(initialData: Data) throws -> String.Encoding {
        ...
        if let rawValue = contentDecoder.detectEncodingRawValue(in: initialData) {
    ...
    static func isProbablyBinary(
        _ data: Data,
        sampleSize: Int = 8192,
        decoder: any FileContentDecoding
    ) -> Bool {
        decoder.isProbablyBinary(Data(data.prefix(sampleSize)))
    }
    ...
    func readDataAndDetectEncoding(_ fullPath: String) throws -> DetectedText {
        ...
        guard let decoded = contentDecoder.decode(data) else {
            throw FileSystemError.failedToReadFile
        }
        return DetectedText(
            string: decoded.string,
