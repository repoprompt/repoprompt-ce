...
package actor IgnoreRulesManager {
    ...
    private let globalIgnoreDefaults: @Sendable () -> String
    private let contentSnapshotReader: any FileContentSnapshotReading
    ...
    private func fileMetaKey(for url: URL) -> FileMetaKey {
        if let fingerprint = try? contentSnapshotReader.fingerprint(atPath: url.path) {
            return FileMetaKey(
                dev: fingerprint.deviceID,
                ino: fingerprint.fileNumber,
                mtime: UInt64(bitPattern: fingerprint.modificationSeconds)
            )
        }
