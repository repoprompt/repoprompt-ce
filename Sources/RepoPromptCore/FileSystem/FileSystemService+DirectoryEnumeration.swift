...
extension FileSystemService {
    ...
    static func enumerateOneLevel(
        ...
        do {
            scan = try listDirectoryWithIgnoreDetection(absFolder, access: directoryAccess)
        } catch {
            ...
            try await withThrowingTaskGroup(of: ScanResult.self) { group in
                ...
                func scheduleMoreTasks() {
                    while inFlight < maxParallel, let folderRel = folderIterator.next() {
                        ...
                        let directoryAccess = self.directoryAccess
                        #if DEBUG
    ...
    func scanOneLevelAndDiff(_ folderRelPath: String) async throws -> [FileSystemDelta] {
        ...
        #if DEBUG
            let scanResult = try Self.listDirectoryWithIgnoreDetection(absFolder, fm: self.fm, access: directoryAccess)
        #else
            let scanResult = try Self.listDirectoryWithIgnoreDetection(absFolder, access: directoryAccess)
        #endif
    ...
    static func buildDirectoryChunk(
        ...
        for entry in scanResult.entries {
            ...
            if isDirEntry {
                ...
                if trackCycles {
                    guard let id = Self.dirID(
                        followingSymlinksAtPath: absolutePath,
                        access: service.directoryAccess
                    ) else {
                        continue
                    }
    ...
    func walkPosixRecursivelyEmitChunks(
        ...
        let rootChain: DirChain?
            ...
            if isVirtualFS || skipSymlinks {
                rootChain = nil
            } else if let rootID = Self.dirID(followingSymlinksAtPath: rootFullPath, access: directoryAccess) {
        ...
        static func processDirectoryOffActor(
            ...
            if testMode {
                ...
                scanResult = try Self.listDirectoryWithIgnoreDetection(context.absPath, fm: fm, access: service.directoryAccess)
            } else {
                scanResult = try Self.listDirectoryWithIgnoreDetection(context.absPath, access: service.directoryAccess)
            }
        ...
        static func processDirectoryOffActor(
            ...
            let scanResult = try Self.listDirectoryWithIgnoreDetection(context.absPath, access: service.directoryAccess)
    ...
    func gatherPathsUsingEnumerator(
        ...
        let rootChain: DirChain?
            ...
            if isVirtualFS || skipSymlinks {
                ...
                var chain: DirChain?
                if let rootID = Self.dirID(followingSymlinksAtPath: canonicalRootPath, access: directoryAccess) {
                ...
                if !baseRelativePath.isEmpty {
                    ...
                    for component in baseRelativePath.split(separator: "/") {
                        ...
                        if let id = Self.dirID(followingSymlinksAtPath: absPath, access: directoryAccess) {
