...
extension FileSystemService {
    ...
    func ensureRulesChain(for relativeDirectory: String, using scanResult: DirectoryScanResult? = nil) async throws -> IgnoreRules {
        ...
        if let provided = scanResult {
            ...
            #if DEBUG
                scan = try Self.listDirectoryWithIgnoreDetection(absPath, fm: fm, access: directoryAccess)
            #else
                scan = try Self.listDirectoryWithIgnoreDetection(absPath, access: directoryAccess)
            #endif
        }
    ...
    func effectiveRules(
        ...
        #if DEBUG
            let scanResult = try Self.listDirectoryWithIgnoreDetection(dirURL.path, fm: fm, access: directoryAccess)
        #else
            let scanResult = try Self.listDirectoryWithIgnoreDetection(dirURL.path, access: directoryAccess)
        #endif
        return try await optimizedEffectiveRules(
            for: dirURL,
