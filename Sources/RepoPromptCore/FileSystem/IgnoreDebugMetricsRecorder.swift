    ...
    package enum IgnoreDebugMetricsRecorder {
        ...
        private static let environmentRecordingEnabled: Bool = {
        ...
        private static var runtimeRecordingEnabled = environmentRecordingEnabled
        private static var runtimeDumpEnabled = isTruthy(ProcessInfo.processInfo.environment[dumpEnabledEnvironmentKey])
        private static var runtimeDumpHandler: (@Sendable (Data) -> Void)?
        private static var testingRecordingOverride: Bool?

        static var isRecordingEnabled: Bool {
            lock.lock()
            defer { lock.unlock() }
            return testingRecordingOverride ?? runtimeRecordingEnabled
        }

        package static func installRuntimePolicy(
            recordingEnabled: Bool,
            dumpEnabled: Bool,
            dumpHandler: @escaping @Sendable (Data) -> Void
        ) {
            lock.lock()
            runtimeRecordingEnabled = environmentRecordingEnabled || recordingEnabled
            runtimeDumpEnabled = isTruthy(ProcessInfo.processInfo.environment[dumpEnabledEnvironmentKey]) || dumpEnabled
            runtimeDumpHandler = dumpHandler
            lock.unlock()
        }

        static func setRecordingEnabledForTesting(_ enabled: Bool) {
            lock.lock()
            testingRecordingOverride = enabled
        ...
        static func dumpSnapshotIfEnabled(label: String) {
            guard let dumpHandler = configuredDumpHandlerIfEnabled() else { return }
        ...
        private static var metricsDumpEnabled: Bool {
            lock.lock()
            defer { lock.unlock() }
            return runtimeDumpEnabled && runtimeDumpHandler != nil
        }
        ...
        private static func configuredDumpHandlerIfEnabled() -> (@Sendable (Data) -> Void)? {
            lock.lock()
            defer { lock.unlock() }
            guard runtimeDumpEnabled else { return nil }
            return runtimeDumpHandler
        }
