...
package struct IgnoreCacheStore {
    package static let finalIgnoreCacheCapacity = 50000

    /// Compact key used by all internal caches – avoids repeated String concatenation.
    package struct PathKey: Hashable {
        let path: String
    ...
    package mutating func isIgnoredPrefixCheck(
        relativePath: String,
    ...
    package func snapshotIgnoreCacheWithPathKeys() -> [PathKey: Bool] {
        ignoreCheckCache.snapshot()
    }

    /// Merge a String-keyed cache (legacy callers) back into our
    /// PathKey-based storage.
    package mutating func mergeIgnoreCache(_ localCache: [String: Bool]) {
    ...
    package mutating func mergeIgnoreCache(_ localCache: [PathKey: Bool]) {
    ...
    package mutating func isIgnoredGlobal(
        _ relPath: String,
        isDirectory: Bool,
        ignoreRules: IgnoreRules
    ) -> Bool {
