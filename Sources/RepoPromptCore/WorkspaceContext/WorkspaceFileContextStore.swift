...
package actor WorkspaceFileContextStore {
    ...
    package func withStoreBackedSearchAccess<T>(
    ...
    package func searchCatalogAccess(rootScope: WorkspaceLookupRootScope = .visibleWorkspace) -> WorkspaceSearchCatalogAccess {
    ...
    package func captureWorkspaceFileContext(
        ...
        coverage requestedCoverage: WorkspaceFileContextCaptureCoverage = .complete
    ) async throws -> WorkspaceFileContextCapture {
        let coverage: WorkspaceFileContextCaptureCoverage = switch fileTreeRequest.mode {
        case .none:
            requestedCoverage
        case .selected, .full, .folders, .auto:
            .complete
        }
        var attempt = 0
        while true {
    ...
    private func makeWorkspaceFileContextCapture(
        ...
        coverage: WorkspaceFileContextCaptureCoverage
        ...
        switch coverage {
        case .complete:
            catalog = searchCatalogSnapshot(rootScope: fileTreeRequest.rootScope)
        case .projection:
            let roots = identity.rootIDs.compactMap { rootStatesByID[$0]?.root }
            let generation = scopedSnapshotGeneration(scope: fileTreeRequest.rootScope)
        ...
        let materializedFolders: [WorkspaceFolderRecord]
        let materializedFiles: [WorkspaceFileRecord]
        let codemapSnapshots: [WorkspaceCodemapSnapshot]
        switch coverage {
        case .complete:
            materializedFolders = foldersByID.values
                .filter { allowedRootIDs.contains($0.rootID) }
                .sorted(by: Self.compareWorkspaceCaptureFolders)
            materializedFiles = filesByID.values
                .filter { allowedRootIDs.contains($0.rootID) }
                .sorted(by: Self.compareWorkspaceCaptureFiles)
            codemapSnapshots = allCodemapSnapshots()
                .filter { allowedRootIDs.contains($0.rootID) }
        case let .projection(codemapCoverage):
            var referencedFolderIDs = Set<UUID>()
            var referencedFileIDs = Set<UUID>()
            func include(_ path: WorkspaceFileContextCapture.SelectionPath) {
                switch path.resolution {
                case let .file(file):
                    referencedFileIDs.insert(file.id)
                case let .folder(folder, descendantFiles):
                    referencedFolderIDs.insert(folder.id)
                    referencedFileIDs.formUnion(descendantFiles.map(\.id))
                case .unresolved:
                    break
                }
            }
            selectedPaths.forEach(include)
            autoCodemapPaths.forEach(include)
            referencedFileIDs.formUnion(slices.compactMap { $0.file?.id })

            switch codemapCoverage {
            case .referenced:
                codemapSnapshots = referencedFileIDs.compactMap { fileID in
                    guard isDiscoverableFileID(fileID),
                          let snapshot = codemapSnapshotsByFileID[fileID],
                          allowedRootIDs.contains(snapshot.rootID)
                    else { return nil }
                    return snapshot
                }
                .sorted { $0.fullPath < $1.fullPath }
            case .allAvailable:
                codemapSnapshots = allCodemapSnapshots()
                    .filter { allowedRootIDs.contains($0.rootID) }
                referencedFileIDs.formUnion(codemapSnapshots.map(\.fileID))
            }

            materializedFolders = referencedFolderIDs.compactMap { foldersByID[$0] }
                .sorted(by: Self.compareWorkspaceCaptureFolders)
            materializedFiles = referencedFileIDs.compactMap { filesByID[$0] }
                .sorted(by: Self.compareWorkspaceCaptureFiles)
        }
        let materializedFolderIDs = Set(materializedFolders.map(\.id))
        let materializedFileIDs = Set(materializedFiles.map(\.id))
        guard prepared.selectedFileIDs.isSubset(of: materializedFileIDs),
    ...
    private static func compareWorkspaceCaptureFolders(
        _ lhs: WorkspaceFolderRecord,
        _ rhs: WorkspaceFolderRecord
    ) -> Bool {
    ...
    private static func compareWorkspaceCaptureFiles(
