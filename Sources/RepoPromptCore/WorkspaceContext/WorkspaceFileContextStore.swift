...
package struct WorkspaceSessionRootLifetimeSnapshot: @unchecked Sendable {
    fileprivate let clock: WorkspaceSessionRootLifetimeClock
    fileprivate let generation: UInt64
    fileprivate let physicalRootPaths: [String]

    package func isGenerationCurrent() -> Bool {
    ...
    package func isCurrent() async -> Bool {
    ...
    @discardableResult
    package func performIfGenerationCurrent(_ operation: () -> Void) -> Bool {
        clock.performIfGenerationCurrent(
            generation: generation,
            operation: operation
        )
...
package actor WorkspaceFileContextStore {
    ...
    private struct WatcherPublisherAttachment {
        ...
        let cancellable: RuntimeObservation
    }
    ...
    private struct ScopedIngressBarrierTarget {
        ...
        package let acceptedServicePublicationSequence: UInt64
    ...
    private final class ScopedIngressBarrierRootFlightState {
    ...
    #if DEBUG
        package struct ScopedIngressBarrierStats: Equatable {
            package let launchCount: Int
            package let joinCount: Int
            package let successorCount: Int
            package let coalescedSuccessorCount: Int
            package let noopCount: Int
        }

        package struct ScopedIngressBarrierDebugSnapshot: Equatable {
            package struct Active: Equatable {
                package let targetWatcherWatermark: UInt64
                package let targetServicePublicationSequence: UInt64
                package let ageMilliseconds: UInt64
            }

            package struct Pending: Equatable {
                package let targetWatcherWatermark: UInt64
                package let targetServicePublicationSequence: UInt64
                package let ageMilliseconds: UInt64
            }

            package struct Completed: Equatable {
            ...
            package let successorCount: Int
            package let coalescedSuccessorCount: Int
            package let completionCount: Int
            package let noopCount: Int
            package let totalWaitMilliseconds: UInt64
            package let maxWaitMilliseconds: UInt64
            package let active: Active?
            package let pending: Pending?
            package let lastCompleted: Completed?
        }

        package struct PublicationInvalidationDebugSample: Equatable {
            package let servicePublicationSequence: UInt64
            package let watcherAcceptedWatermark: UInt64?
            package let preparedDeltaCount: Int
            package let topologyInvalidationCount: Int
            package let catalogGenerationAdvanceCount: Int
            package let searchCatalogCacheClearCount: Int
            package let pathWorkerInvalidationRequestCount: Int
            package let contentInvalidationCount: Int
            package let distinctContentKeyCount: Int
            package let decodedCacheInvalidationRequestCount: Int
            package let codemapInvalidationRequestCount: Int
            package let appliedIndexEventYieldCount: Int
        }

        package struct PublicationInvalidationHistoryDebugSnapshot: Equatable {
            package let retainedSampleLimit: Int
            package let totalObservedPublicationCount: Int
            package let droppedPublicationSampleCount: Int
            package let samples: [PublicationInvalidationDebugSample]
        }

        package struct CatalogInvalidationDebugEvent: Equatable {
            package let sequence: UInt64
            package let reasons: [String]
            package let affectedRootIDs: [UUID]
            package let affectedRootKinds: [String]
            package let evictedScopes: [String]
        }

        package struct CatalogRebuildDebugSnapshot: Equatable {
            ...
            package let lastFileCount: Int
            package let lastRootCount: Int
        }

        package enum RootCatalogShardFallbackReason: String, CaseIterable, Hashable {
        ...
        package struct RootCatalogShardGenerationDebugSnapshot: Equatable {
            package let rootID: UUID
            package let lifetimeID: UUID?
            package let publishedTopologyGeneration: UInt64?
            package let liveTopologyGenerations: [UInt64]
            package let retainedTopologyGenerations: [UInt64]
            package let buildCount: Int
            package let pathIndexBuildCount: Int
            package let overlayPathIndexBuildCount: Int
            package let patchCount: Int
            package let authoritativeRebuildCount: Int
            package let fallbackCount: Int
            package let fallbackReasonCounts: [RootCatalogShardFallbackReason: Int]
            package let lastAppliedIndexGeneration: UInt64?
            package let deltaStateDirty: Bool
            package let backstopCount: Int
            package let maxLiveGenerationCount: Int
        }

        package struct RootCatalogShardDebugSnapshot: Equatable {
            package let liveGenerationCapPerRoot: Int
            package let maxPatchLogicalMutationCount: Int
            package let maxPathIndexOverlayChangedFileCount: Int
            package let publishedShardCount: Int
            package let totalBuildCount: Int
            package let totalBackstopCount: Int
            package let singleShardCompositionReuseCount: Int
            package let genericMergeElementVisitCount: Int
            package let shadowComparisonCount: Int
            package let shadowMismatchCount: Int
            package let lastShadowByteCount: Int
            package let roots: [RootCatalogShardGenerationDebugSnapshot]
        }

        package struct StoreWorkDiagnosticsSnapshot: Equatable {
            package let invalidations: [CatalogInvalidationDebugEvent]
            package let catalogRebuild: CatalogRebuildDebugSnapshot
            package let rootCatalogShards: RootCatalogShardDebugSnapshot
        }

        package struct ReadSearchRootDiagnosticsSnapshot: Equatable {
            package let rootID: UUID
            package let rootToken: UUID
            package let rootPath: String
            package let rootKind: String
            package let crawlCount: Int
            package let watcherActive: Bool
            package let explicitWatcherDemand: Bool
            package let sessionWorktreeOwnerCount: Int
            package let ingress: WorkspaceFileSystemIngressCoordinator.DebugSnapshot
            package let barrier: ScopedIngressBarrierDebugSnapshot
            package let freshness: FileSystemService.FreshnessWorkDiagnosticsSnapshot
            package let invalidation: PublicationInvalidationHistoryDebugSnapshot
            package let producedAppliedIndexGeneration: UInt64
        }

        package struct ApplyEditsRebaseProbePathSnapshot: Equatable {
            package let rootID: UUID
            ...
            package let relativePath: String
            package let isSessionWorktree: Bool
            package let producedAppliedIndexGeneration: UInt64
            ...
            package init(
                ...
                fullPath: String,
                relativePath: String,
                isSessionWorktree: Bool,
                producedAppliedIndexGeneration: UInt64
            ) {
                self.rootID = rootID
                ...
                self.isSessionWorktree = isSessionWorktree
                self.producedAppliedIndexGeneration = producedAppliedIndexGeneration
            }
        }
        ...
        package func setRootLoadWillStartHandler(_ handler: (@Sendable (String) async -> Void)?) {
            rootLoadWillStartHandler = handler
        }

        package func setRootLoadDidJoinInFlightHandler(_ handler: (@Sendable (String) async -> Void)?) {
            rootLoadDidJoinInFlightHandler = handler
        }

        package func setRootUnloadDidDetachHandler(_ handler: (@Sendable ([String]) async -> Void)?) {
        ...
        package func setWatcherSinkWillApplyHandler(_ handler: (@Sendable (UUID) async -> Void)?) {
            watcherSinkWillApplyHandler = handler
        }

        package func setPublisherIngressWillWaitHandler(_ handler: (@Sendable (Set<UUID>) async -> Void)?) {
            publisherIngressWillWaitHandler = handler
        }

        package func setWatcherPublisherIngressDidOpenHandler(_ handler: (@Sendable (UUID, UUID) async -> Void)?) {
            watcherPublisherIngressDidOpenHandler = handler
        }

        package func setWatcherInfrastructureDidJoinFlightHandler(_ handler: (@Sendable (UUID, UUID) async -> Void)?) {
            watcherInfrastructureDidJoinFlightHandler = handler
        }

        package func watcherInfrastructureFlightCountForTesting(rootID: UUID) -> Int {
            watcherInfrastructureFlightsByKey.keys.count(where: { $0.rootID == rootID })
        }

        package func setWatcherServiceStateWillReconcileHandler(_ handler: (@Sendable (UUID, Bool) async -> Void)?) {
            watcherServiceStateWillReconcileHandler = handler
        }

        package func setWatcherStopWillBeginHandler(_ handler: (@Sendable (UUID) async -> Void)?) {
        ...
        package func setAppliedIngressDidCaptureWatermarksHandler(_ handler: (@Sendable ([UUID: UInt64]) async -> Void)?) {
            appliedIngressDidCaptureWatermarksHandler = handler
        }

        package func setScopedIngressBarrierWillFlushHandler(_ handler: (@Sendable (UUID) async -> Void)?) {
            scopedIngressBarrierWillFlushHandler = handler
        }

        package func setWatcherActivationFailureForNewServicesForTesting(
        ...
        package func scopedIngressBarrierStatsForTesting(rootID: UUID) -> ScopedIngressBarrierStats {
        ...
        package func scopedIngressBarrierFlightCountForTesting() -> Int {
            scopedIngressBarrierFlightStatesByRootID.values.reduce(0) { count, state in
                count + (state.active == nil ? 0 : 1) + (state.pending == nil ? 0 : 1)
            }
        }

        package func fileSystemServiceForTesting(rootID: UUID) -> FileSystemService? {
            rootStatesByID[rootID]?.service
        }

        package func debugApplyEditsRebaseProbePathSnapshot(
        ...
        package func readSearchRootDiagnosticsSnapshot(
        ...
        package func storeWorkDiagnosticsSnapshot() -> StoreWorkDiagnosticsSnapshot {
        ...
        package func retainedReadSearchDiagnosticRootIDsForTesting() -> Set<UUID> {
        ...
        package func searchDecodedContentCacheSnapshotForTesting() async -> WorkspaceSearchDecodedContentCache.Snapshot {
            await searchDecodedContentCache.snapshotForTesting()
        }

        package func interactiveReadCacheSnapshotForTesting() async -> WorkspaceInteractiveReadCache.Snapshot {
            await interactiveReadCache.snapshotForTesting()
        }

        package func searchLaneSnapshotForTesting() async -> StoreBackedWorkspaceSearchLane.Snapshot {
        ...
        package func setSearchLanePermitAcquiredHandlerForTesting(
            _ handler: (@Sendable () async -> Void)?
        ) async {
            await storeBackedSearchLane.setPermitAcquiredHandlerForTesting(handler)
        }

        package func setStoreEditDeferredPublicationDidRegisterHandlerForTesting(
            _ handler: (@Sendable (UUID, String) async -> Void)?
        ) {
            storeEditDeferredPublicationDidRegisterHandler = handler
        }

        package func setSearchContentReadChunkHandlerForTesting(
            rootID: UUID,
        ...
        package func resetSearchContentFingerprintRequestCountForTesting(rootID: UUID) async throws {
            let state = try state(for: rootID)
            await state.service.resetContentFingerprintRequestCountForTesting()
        }

        package func searchContentFingerprintRequestCountForTesting(rootID: UUID) async throws -> Int {
            let state = try state(for: rootID)
            return await state.service.contentFingerprintRequestCountSnapshotForTesting()
        }

        package func setCachedSearchContentWatcherActiveOverrideForTesting(
            rootID: UUID,
            _ isActive: Bool?
        ) async throws {
            let state = try state(for: rootID)
    ...
    #if DEBUG
        package init(
            runtimeDependencies: WorkspaceRuntimeDependencies,
            ...
            WorkspaceRuntimePerf.installProcessSink(runtimeDependencies.diagnostics)
            codeScanActor = CodeScanActor(cacheRoot: runtimeDependencies.configuration.runtimePaths.codeMapCacheRoot)
    ...
    deinit {
        ...
        for attachment in watcherPublisherAttachmentsByKey.values {
            publisherIngressCoordinator.closePublisherIngress(attachment.subscription)
    ...
    private func attachPublisherIngress(
        ...
        let cancellable = publisher.sink { publication in
                ...
                let publicationCorrelation = WorkspaceRuntimePerf.currentFileSystemPublicationCorrelation
            #else
                let publicationCorrelation: WorkspaceRuntimePerf.LifecycleCorrelation? = nil
            #endif
            WorkspaceRuntimePerf.lifecycleEvent(
                WorkspaceRuntimePerf.Lifecycle.WorkspaceIngress.storeSinkScheduled,
                correlation: publicationCorrelation,
                WorkspaceRuntimePerf.Dimensions(
                    changeCount: publication.deltas.count,
                    rootToken: diagnosticRootToken.uuidString,
    ...
    #if DEBUG
        package func attachPublisherIngressWithoutStartingWatcherForTesting(rootID: UUID) async throws -> Bool {
            let state = try state(for: rootID)
            let key = WatcherInfrastructureKey(rootID: rootID, lifetimeID: state.lifetimeID)
            if let attachment = watcherPublisherAttachmentsByKey[key],
               publisherIngressCoordinator.isPublisherIngressOpen(attachment.subscription)
    ...
    package func prepareSessionWorktreeOwnership(
        ownerID: UUID,
    ...
    @discardableResult
    package func commitSessionWorktreeOwnership(
    ...
    package func abortSessionWorktreeOwnership(
        _ preparation: WorkspaceSessionWorktreeOwnershipPreparation
    ) async {
    ...
    package func releaseSessionWorktreeOwnership(ownerID: UUID) async {
    ...
    package func sessionWorktreeOwnershipCovers(
        ownerID: UUID,
        bindingFingerprint: String,
    ...
    package func installedSessionWorktreeRoots(
        ownerID: UUID,
        bindingFingerprint: String,
        physicalRootPaths: Set<String>
    ) -> [WorkspaceRootRecord]? {
    ...
    #if DEBUG
        package struct SessionWorktreeOwnershipDebugSnapshot: Equatable {
            package let installedOwnerCount: Int
            package let provisionalOwnerCount: Int
            package let rootClaimCount: Int
            package let pathReservationCount: Int
            package let explicitWatcherDemandCount: Int
        }

        package func sessionWorktreeOwnershipDebugSnapshotForTesting() -> SessionWorktreeOwnershipDebugSnapshot {
    ...
    package func replayObservedFileSystemDeltas(rootID: UUID, deltas: [FileSystemDelta]) async {
    ...
    #if DEBUG
        package func replayFileSystemPublicationForInvalidationDiagnosticsForTesting(
            rootID: UUID,
        ...
        package func publishSyntheticFileSystemDeltasForTesting(rootID: UUID, deltas: [FileSystemDelta]) async throws {
            let state = try state(for: rootID)
            await state.service.publishFileSystemDeltas(deltas, source: .syntheticMutation)
        }

        package func publisherIngressCountForTesting(rootID: UUID) -> Int {
            publisherIngressCoordinator.pendingPublisherIngressCount(rootIDs: [rootID])
        }

        package func rootWatcherIsActiveForTesting(rootID: UUID) async throws -> Bool {
            let state = try state(for: rootID)
            return await state.service.isWatchingForChangesForTesting()
        }

        package func acceptWatcherPayloadForTesting(
            rootID: UUID,
        ...
        package func appliedIngressSnapshotForTesting(rootID: UUID) -> WorkspaceFileSystemIngressCoordinator.AppliedSnapshot {
            publisherIngressCoordinator.appliedSnapshot(rootID: rootID)
        }

        package func acceptedWatcherWatermarkForTesting(rootID: UUID) throws -> FileSystemWatcherIngressMailbox.Watermark {
            try state(for: rootID).service.captureAcceptedWatcherWatermark()
        }

        package func publisherIngressDebugSnapshotForTesting(
            rootID: UUID
        ) -> WorkspaceFileSystemIngressCoordinator.DebugSnapshot {
            publisherIngressCoordinator.debugSnapshot(rootID: rootID)
        }

        package func waitUntilPublisherIngressAppliedForTesting(
            rootID: UUID,
            servicePublicationSequence: UInt64
        ) async {
        ...
        package func rootLifetimeIDForTesting(rootID: UUID) throws -> UUID {
        ...
        package func replayPublisherFileSystemPublicationForTesting(
            rootID: UUID,
    ...
    package func appliedIndexRootSnapshot(rootID: UUID) -> WorkspaceAppliedIndexRootSnapshot? {
    ...
    package struct ReadFileAutoSelectionCatalogValidationSnapshot: Equatable {
        package let visibleCatalogGeneration: UInt64
        package let rootScopeCatalogGeneration: UInt64
        package let rootScopeAvailability: WorkspaceLookupRootScopeAvailability
    }

    package func readFileAutoSelectionCatalogValidationSnapshot(
        rootScope: WorkspaceLookupRootScope
    ...
    package func sessionBoundRootScopeValidationSnapshot(
    ...
    package func rootScopeAvailability(_ rootScope: WorkspaceLookupRootScope) -> WorkspaceLookupRootScopeAvailability {
        let missing: [String]
        switch rootScope {
        case .visibleWorkspace, .visibleWorkspacePlusGitData, .allLoaded, .allLoadedExcludingGitData:
            return .available
        ...
        package func debugAuthoritativeCatalogSortProbe(
            rootScope: WorkspaceLookupRootScope,
            warmupCount: Int = 1,
            measuredCount: Int = 3
    ...
    package func searchCatalogSnapshot(
        rootScope: WorkspaceLookupRootScope = .visibleWorkspace,
        requirement: WorkspaceSearchCatalogAccessRequirement = .recordsAndPathIndexes
    ...
    package func contentSearchFreshnessPolicy(
        rootScope: WorkspaceLookupRootScope,
    ...
    package func codemapFileAPIAggregate(rootScope: WorkspaceLookupRootScope) -> WorkspaceCodemapFileAPIAggregate {
    ...
    @discardableResult
    package func invalidateCodemapSnapshotsForCheckoutMutation(rootIDs: [UUID]) -> [UUID] {
    ...
    @discardableResult
    package func reconcileLoadedRootCatalogWithDisk(rootID: UUID) async -> [FileSystemDelta] {
        guard let state = rootStatesByID[rootID] else { return [] }
        let root = state.root
        let folderPaths = Set(
    ...
    package func cancelCodemapScansForCheckoutMutation(rootIDs: [UUID]) async {
    ...
    package func purgeStaleCodemapCaches(keepingRootPaths: [String]) async {
        ...
        package func setCodemapScanWillStartHandlerForTesting(
            _ handler: (@Sendable (UUID) async -> Void)?
        ) async {
    ...
    package func loadRoot(
        ...
        #if DEBUG
            let rootLoadRouteStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
            ...
            guard let existingConfiguration = rootLoadConfigurationsByPath[standardizedPath], existingConfiguration == loadConfiguration else {
                ...
                WorkspaceRuntimeDiagnosticsLog.event(
                    ...
                    fields: [
                        ...
                        "rootID": WorkspaceRuntimeDiagnosticsLog.shortID(existing.id),
                        "kind": "\(loadConfiguration.kind)",
                        "duration": rootLoadRouteStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                    ]
                )
            #endif
            return existing
    ...
    private func performLoadRoot(
        ...
        #if DEBUG
            let performLoadStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
            WorkspaceRuntimeDiagnosticsLog.event(
        ...
        let service = try await FileSystemService(
            ...
            var rootRecordCreatedFields: [String: String] = [
                "rootName": root.name,
                "rootID": WorkspaceRuntimeDiagnosticsLog.shortID(root.id),
                "kind": "\(root.kind)",
                "durationSinceStoreRootLoadBegin": performLoadStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
            ]
            ...
            WorkspaceRuntimeDiagnosticsLog.event("store.rootLoad.rootRecordCreated", fields: rootRecordCreatedFields)
        #endif
        ...
        #if DEBUG
            let walkStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
            var chunkCount = 0
        #endif
        for try await event in await service.loadContentsInChunks(of: rootURL) {
            ...
            guard case let .preparedItems(chunk) = event else { continue }
                ...
                if chunkCount == 1 {
                    ...
                    WorkspaceRuntimeDiagnosticsLog.event("store.rootLoad.firstPreparedChunk", fields: firstChunkFields)
                }
            #endif
        ...
        try Task.checkCancellation()
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    ...
                    "duration": walkStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
            let commitStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        ...
        commit(stagedIndexes)
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    ...
                    "folders": "\(stagedIndexes.foldersByID.count)",
                    "files": "\(stagedIndexes.filesByID.count)",
                    "duration": commitStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
        #endif
        ...
        invalidatePathMatchSnapshot(
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    ...
                    "folders": "\(stagedIndexes.foldersByID.count)",
                    "files": "\(stagedIndexes.filesByID.count)",
                    "duration": performLoadStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
        #endif
        return root
    ...
    package func unloadRoots(ids rootIDs: [UUID]) async {
        ...
        for entry in statesToUnload {
        ...
        #if DEBUG
            let rootUnloadStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
            ...
            let detachStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        ...
        for entry in statesToUnload {
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    "rootCount": "\(statesToUnload.count)",
                    "duration": detachStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
            let stopWatchersStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        ...
        WorkspaceRootUnloadDiagnosticsLog.record(terminationDiagnostics)
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    "rootCount": "\(statesToUnload.count)",
                    "duration": stopWatchersStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
            let indexCleanupStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        ...
        for entry in statesToUnload {
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    ...
                    "duration": indexCleanupStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
            let codeScanCancelStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        await codeScanActor.cancelAndUnloadScans(forRootFolders: rootPathsToUnload)
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    "rootCount": "\(statesToUnload.count)",
                    "duration": codeScanCancelStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
        #endif
        ...
        await cleanupOrphanedSessionWorktreeResources(SessionWorktreeOwnershipRemoval(
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    "rootCount": "\(statesToUnload.count)",
                    "duration": rootUnloadStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
        #endif
    }
    ...
    package func cachedSearchContentSnapshot(
    ...
    package func interactiveReadSnapshot(
        for expectedRecord: WorkspaceFileRecord
    ...
    package func readValidatedContentSnapshot(
        rootID: UUID,
        relativePath: String,
        workloadClass: ContentReadWorkloadClass = .unspecified
    ...
    @discardableResult
    package func initializeCodemapsForSessionWorktreeRoots(rootIDs: [UUID]) -> [UUID] {
        var seen = Set<UUID>()
    ...
    package func enqueueMissingCodemapSnapshotRepairs(
    ...
    package func repairMissingCodemapSnapshots(
        for files: [WorkspaceFileRecord],
    ...
    package func requestInitialRootCodemapScans(
        ...
        await ensureCodeScanResultTask()
        #if DEBUG
            let collectFilesStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        ...
        for rootPath in standardizedRootPaths {
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    ...
                    "duration": collectFilesStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
            let buildRequestsStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        let requests = try await codemapScanRequests(for: filesToScan)
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    ...
                    "duration": buildRequestsStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
            let submitStartMS = WorkspaceRuntimeDiagnosticsLog.timestampMSIfEnabled()
        #endif
        await codeScanActor.requestScans(
            ...
            WorkspaceRuntimeDiagnosticsLog.event(
                ...
                fields: [
                    ...
                    "requests": "\(requests.count)",
                    "rootCount": "\(standardizedRootPaths.count)",
                    "duration": submitStartMS.map { WorkspaceRuntimeDiagnosticsLog.formatElapsedMS(since: $0) } ?? "notMeasured"
                ]
            )
        #endif
    }
    ...
    package func ingressPublishedGitArtifacts(
    ...
    package func exactRootRef(path: String, kind: WorkspaceRootKind) -> WorkspaceRootRef? {
    ...
    #if DEBUG
        package func staticPathMatchSnapshotCacheCountForTesting() -> Int {
            staticPathMatchSnapshotsByScope.count
        }

        package func searchCatalogSnapshotCacheCountForTesting() -> Int {
            searchCatalogSnapshotsByScope.count
        }

        package func sessionCatalogGenerationForTesting(scope: WorkspaceLookupRootScope) -> UInt64? {
    ...
    package func sliceRebasePathState(fullPath rawFullPath: String) -> WorkspaceSliceRebasePathState? {
        let fullPath = StandardizedPath.absolute(rawFullPath)
    ...
    package func sliceRebaseFileIsCurrent(
        rootID: UUID,
        rootLifetimeID: UUID,
        fileID: UUID,
        relativePath: String,
