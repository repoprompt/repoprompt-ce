...
package struct DeferredReplayPendingWorkSnapshot: Equatable {
    ...
    package let overflowedRootCount: Int
}
    ...
    package struct DeferredReplayBufferDiagnostics: Equatable {
        ...
        package let overflowedRootCount: Int
        package let routingVersion: UInt64
        package let immediatePreparedIngressInFlight: Bool
        package let immediateIngressCount: UInt64
        package let deferredIngressCount: UInt64
        package let overflowIngressCount: UInt64
...
package actor DeferredReplayBufferActor {
    ...
    package func updateRoutingState(
    ...
    package func updateImmediateReplayChunkSizeOverride(_ chunkSize: Int?) {
        if let chunkSize {
    ...
    package func registerActiveRootGeneration(_ generation: UInt64, forRootKey rootKey: String) {
        activeRootGenerations[rootKey] = generation
        clearBufferedState(forRootKey: rootKey)
    }

    package func unregisterActiveRootGeneration(forRootKey rootKey: String) {
        activeRootGenerations.removeValue(forKey: rootKey)
        clearBufferedState(forRootKey: rootKey)
    }

    package func ingestLiveDeltas(
    ...
    package func finishPreparedImmediateIngress(_ immediateReplay: PreparedImmediateReplay) {
        releaseImmediatePreparedIngressReservation(immediateReplay.reservation)
    }

    package func enqueueDeferredDeltas(
        _ deltas: [FileSystemDelta],
        forRootKey rootKey: String
    ...
    package func drainPreparedBatches(
    ...
    package func clearRoot(_ rootKey: String) {
        clearBufferedState(forRootKey: rootKey)
    }
    ...
    package func hasPendingWork() -> Bool {
        !pendingDeltasByRoot.isEmpty
    }

    package func pendingDeltaCount(forRootKey rootKey: String) -> Int {
        pendingDeltasByRoot[rootKey]?.count ?? 0
    }

    package func pendingWorkSnapshot() -> DeferredReplayPendingWorkSnapshot {
    ...
    #if DEBUG
        package func diagnosticsSnapshot() -> DeferredReplayBufferDiagnostics {
