...
package struct WorkspaceInteractiveReadCacheKey: Hashable {
    ...
    package init(rootID: UUID, rootLifetimeID: UUID, fileID: UUID, standardizedRelativePath: String) {
        self.rootID = rootID
...
package struct WorkspaceInteractiveReadPreparedContent: Equatable {
    package let linesWithEndings: [String]
...
package struct WorkspaceInteractiveReadSlice: Equatable {
    package let content: String
    package let totalLines: Int
    package let firstLine: Int
    package let lastLine: Int
    package let returnedLineCount: Int
    package let startExceededFileLength: Bool
}

package enum WorkspaceInteractiveReadRangeError: Error, Equatable {
    case limitWithNegativeStart
    case zeroStart
}

package enum WorkspaceInteractiveReadProcessor {
    package static func prepareOffActor(_ content: String) async -> WorkspaceInteractiveReadPreparedContent {
    ...
    package static func sliceOffActor(
    ...
    package static func prepare(_ content: String) -> WorkspaceInteractiveReadPreparedContent {
        WorkspaceInteractiveReadPreparedContent(
            linesWithEndings: StringLineUtilities.splitPreservingAllLineEndings(content).map { pair in
                pair.line + pair.ending
            }
        )
    }

    package static func slice(
        _ prepared: WorkspaceInteractiveReadPreparedContent,
        startLine1Based: Int?,
        lineCount: Int?
...
package struct WorkspaceInteractiveReadCacheLookup {
    let preparedContent: WorkspaceInteractiveReadPreparedContent?
    let cacheHit: Bool
}

package struct WorkspaceInteractiveReadSnapshot {
    package let preparedContent: WorkspaceInteractiveReadPreparedContent
    package let cacheHit: Bool
}

package actor WorkspaceInteractiveReadCache {
    #if DEBUG
        package struct Snapshot: Equatable {
            package let entryCount: Int
            package let activeFlightCount: Int
            package let waiterCount: Int
            package let estimatedCost: Int
            package let hitCount: Int
            package let preparationCount: Int
            package let joinCount: Int
            package let cancellationCount: Int
            package let acceptedPreparationCount: Int
        }
    #endif
    ...
    package init(maxEntryCount: Int = 512, maxEstimatedCost: Int = 64 * 1024 * 1024) {
