...
package final class WorkspaceFileSystemIngressCoordinator: @unchecked Sendable {
    ...
    package struct AppliedSnapshot: Equatable {
        ...
        package let appliedWatcherWatermark: FileSystemWatcherIngressMailbox.Watermark
    }

    package enum TerminationOutcome: String, Equatable {
        case graceful
        case forced
        case missing
        case superseded
    ...
    package struct TerminationReport: Equatable {
        package let rootID: UUID
        package let stateIdentity: UUID?
        package let outcome: TerminationOutcome
        package let graceNanoseconds: UInt64
        package let queuedPublicationCount: Int
        package let applyingPublicationCount: Int
        package let waiterCount: Int
        package let acceptedServicePublicationSequence: UInt64
        package let appliedServicePublicationSequence: UInt64
        package let acceptedAppliedSequenceGap: UInt64
        package let appliedWatcherWatermark: UInt64
        package let oldestOutstandingPublicationAgeMilliseconds: UInt64?
    }
        ...
        package struct DebugSnapshot: Equatable {
            ...
            package let queuedPublicationCount: Int
            package let applyingPublicationCount: Int
            package let outstandingPublicationCount: Int
            package let waiterCount: Int
            package let acceptedServicePublicationSequence: UInt64
            package let appliedServicePublicationSequence: UInt64
            package let acceptedAppliedSequenceGap: UInt64
    ...
    typealias DrainHandler = @Sendable (FileSystemDeltaPublication, WorkspaceRuntimePerf.LifecycleCorrelation?) async -> Void
    ...
    private final class RootState {
        ...
        var applyingCount = 0
        package var acceptedServicePublicationSequence: UInt64 = 0
        package var appliedServicePublicationSequence: UInt64 = 0
        package var appliedWatcherWatermark = FileSystemWatcherIngressMailbox.Watermark.zero
