...
package enum StoreBackedWorkspaceSearchError: LocalizedError, Equatable {
...
package enum StoreBackedWorkspaceSearch {
    private static let workspaceFreshnessWaitTimeout: Duration = .seconds(30)
    private static let workspaceReadinessWaitTimeout: Duration = .seconds(30)
    #if DEBUG
        @TaskLocal package static var readinessWaitTimeoutOverrideForTesting: Duration?
        @TaskLocal package static var freshnessWaitTimeoutOverrideForTesting: Duration?
        @TaskLocal package static var freshnessWaitOperationOverrideForTesting: (@Sendable ([WorkspaceRootRef], WorkspaceFileContextStore) async -> [WorkspaceIngressBarrierSample])?
    #endif
    ...
    package static func search(
        ...
        workspaceManager: (any WorkspaceSearchReadinessSource)?
        ...
        return try await store.withStoreBackedSearchAccess(
            ...
            try await validateSearchReadiness(readinessTicket, workspaceManager: workspaceManager)
                ...
                let freshnessWaitTimeout = freshnessWaitTimeoutOverrideForTesting
                    ?? workspaceFreshnessWaitTimeout
            #else
                let freshnessWaitTimeout = workspaceFreshnessWaitTimeout
            #endif
    ...
    private static func ensureRootScopeAvailable(
        ...
        workspaceManager: (any WorkspaceSearchReadinessSource)? = nil
    ) async throws {
        let perfState = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.Search.rootScopeAvailabilityGate)
        defer { WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.Search.rootScopeAvailabilityGate, perfState) }
    ...
    private static func acquireSearchReadiness(
        ...
        let perfState = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.Search.workspaceReadinessAcquireGate)
        defer { WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.Search.workspaceReadinessAcquireGate, perfState) }
        guard let workspaceManager else {
            ...
            let timeout = readinessWaitTimeoutOverrideForTesting
                ?? workspaceReadinessWaitTimeout
        #else
            let timeout = workspaceReadinessWaitTimeout
        #endif
    ...
    private static func validateSearchReadiness(
        ...
        let perfState = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.Search.workspaceReadinessValidationGate)
        defer { WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.Search.workspaceReadinessValidationGate, perfState) }
        guard let workspaceManager else {
