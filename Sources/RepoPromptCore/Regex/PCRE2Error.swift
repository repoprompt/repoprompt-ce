...
public enum PCRE2Error: Error, LocalizedError, Sendable, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case let .compile(pattern, offset, code, message):
            "PCRE2 compile error at byte offset \(offset) for pattern \(String(reflecting: pattern)) (\(code)): \(message)"
        case let .match(code, message):
            "PCRE2 match error (\(code)): \(message)"
        case let .matchLimitExceeded(kind, code, message):
            "PCRE2 match limit exceeded (\(kind.description), \(code)): \(message)"
        case let .jitRequiredButUnavailable(message):
            "PCRE2 JIT required but unavailable: \(message)"
        case let .internalInvariant(message):
            "PCRE2 wrapper invariant failed: \(message)"
        }
    }
}

func pcre2ErrorMessage(_ code: Int32) -> String {
