...
This calls `Scripts/test_suite_optimizer.py impacted` with the curated ledger. Its default change set unions the merge-base branch diff, staged and unstaged changes, and untracked files. It prints exact selected XCTest IDs with reasons, includes a small smoke-floor suite set, reports heavy/opt-in tests skipped by `execution_tier`, and invokes conductor once with an exact XCTest filter. The selector is conservative: broad build/tooling/package/ledger boundaries report `full_root_required=true` instead of pretending a narrow subset is sufficient. `FULL_FALLBACK=1` runs the authoritative full root suite in that case; `pr-ready` enables this fallback. Set `INCLUDE_HEAVY=1` only when intentionally running selected `codemap_e2e`, `scale`, `diagnostic`, `live_smoke`, or `release` rows and after setting any required environment gate.
...
For ordinary Swift style iteration, `make format-impacted`, `make format-check-impacted`, and `make lint-impacted` use one invocation per selected tool over the same default branch/staged/unstaged/untracked union. Explicitly excluded vendored, generated, transient, and workflow-prompt files remain excluded. Changes to style configuration, manifests, or style wiring fall back to the full configured Swift scope. Full coordinated `make dev-format-check` and `make dev-lint` remain the explicit authoritative style lanes, and hosted CI continues to run the full lint and full sharded root suite.
