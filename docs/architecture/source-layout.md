...
- Reusable PCRE2 wrapper/JIT construction belongs under `Packages/RepoPromptPortableRuntime/Sources/RepoPromptRegexCore`; app search policy, limits, repair, and presentation remain app-owned.
...
- Provider-neutral workflow IDs, catalog metadata, and prompt renderers go under `Packages/RepoPromptPortableRuntime/Sources/RepoPromptShared/Workflows/`; do not add new workflow prompts under provider-specific command names or bundled `AppResources/Services/AI/Prompts` mirrors.
...
- New app/CLI protocol definitions shared by both executables go under `Packages/RepoPromptPortableRuntime/Sources/RepoPromptShared`.
- MCP filesystem/product/build-flavor identity and external-client event wire DTOs are single-sourced under `Packages/RepoPromptPortableRuntime/Sources/RepoPromptShared/MCP`; app/helper targets may keep only local compile-flavor selection and app-only presentation behavior.
...
- `MCPControlMessages.swift` and `MCPFilesystemIdentity.swift` exist only under `Packages/RepoPromptPortableRuntime/Sources/RepoPromptShared/MCP`, and the `MCPExternalClientEvent` wire DTO is declared only there;
- parser fixtures/sample inputs do not live under app syntax parsing source;
- the RepoPrompt `SwiftTreeSitter` fork revision, exact runtime, and complete grammar requirement/resolved-revision set remain pinned by the portable manifest and lockfile, `RepoPromptCodeMapCore` imports the grammar modules directly, `SyntaxManager` consumes the portable CodeMap product for query compilation and validation, retired local grammar directories remain absent, and the narrow `TreeSitterScannerSupport` target contains only its approved exact-snapshot files with matching checksums;
...
- `MCPControlMessages.swift`, `MCPFilesystemIdentity.swift`, and the `MCPExternalClientEvent` wire DTO now have one source of truth in `Packages/RepoPromptPortableRuntime/Sources/RepoPromptShared/MCP`; the app and CLI targets depend on `RepoPromptShared`.
...
- Workflow prompt generation now lives in the provider-neutral catalog under `Packages/RepoPromptPortableRuntime/Sources/RepoPromptShared/Workflows/`; the old provider-specific `ClaudeCodeCommands` surface and duplicated bundled prompt mirror under `AppResources/Services/AI/Prompts/` should not be restored.
