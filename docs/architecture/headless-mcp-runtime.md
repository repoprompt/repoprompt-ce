...
Direct Context Builder and Oracle runs use the persisted `context_builder.context_token_budget` and `context_builder.analysis_token_budget` settings. The former defaults to the app's 160k context-only budget and the latter to its 120k plan/review/question budget; operators can set bounded 5k-aligned values through `app_settings` (for example, 100k/150k). The direct worker receives both hard maxima, the configured Context Builder agent/model is honored, and recursive AI/delegation calls are prohibited. Provider JSON is parsed incrementally with a bounded per-line buffer so large command events cannot hide the final assistant result or create quadratic parsing work. When a caller supplies standard MCP request progress, direct mode publishes bounded provider milestones plus a ten-second liveness heartbeat until terminal completion.
...
`REPOPROMPT_MCP_HEADLESS_PROFILE_DIR` is an explicit isolation boundary for tests and automation. A nondefault `REPOPROMPT_MCP_HEADLESS_PROFILE` requires that directory instead of inventing storage. Explicit roots may bootstrap a synthetic workspace only inside such an isolated profile. Protected mutations default to deny until the selected persistence policy authorizes the verified principal. Trusted, operator-owned MCP launch configuration may instead set `REPOPROMPT_MCP_HEADLESS_GRANT_OPERATIONS` to a comma-separated set of exact `tool.action` operations; those grants apply only to the verified top-level client and last for that MCP process lifetime, without modifying persistent policy. Empty components, wildcards, and malformed operations fail startup. Long-running provider costs remain decorated and auditable. Direct mode has no AppKit, SwiftUI, window, view-model, live-app, or UI-presentation dependency.
