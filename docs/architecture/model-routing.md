...
The routing envelope has a local 4,000-character / 16-KiB task-text ceiling; it is a RepoPrompt disclosure and latency guard, not Jev's context limit. An over-limit task is not sent to Jev. Router reports the local limit and asks for a shorter task or a manual target; explicitly pinned MCP starts bypass Router and can still send the full task to their selected provider. Do not increase this ceiling merely because Jev accepts more tokens: longer raw prompts can expose private material and distract the routing judgment.
