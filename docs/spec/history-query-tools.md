...
- Active duration is derived at query time from merged provider-response intervals plus discrete request, activity, summary, and turn-boundary observations. Only gaps between those observations/intervals that are less than or equal to `idle_threshold_minutes` are counted as active. This prevents a single turn that pauses for approval and resumes hours or days later from counting the entire wall-clock wait as work. The idle threshold is an integer in the inclusive range 0–1440 minutes. An omitted or null value uses the settings-backed app default (`history.idle_threshold_minutes`, currently 10 minutes when unset), while an out-of-range or non-integer value is a validation error (not clamped). An out-of-range stored default (for example, written out-of-band via `defaults`) is clamped to 0–1440. A gap strictly greater than the threshold is idle and excluded; a gap less than or equal is active. A threshold of 0 means every positive gap is idle, so `active_duration_seconds` equals only merged provider-response coverage. The same threshold applies to every active-duration field an operation returns (`total_active_duration_seconds`, per-group, and per-detail). The gap between observations is an approximation of idle time — it conflates user think-time, agent pauses, and time away from the app; it is not strictly "time waiting for user input."
- Cross-workspace inventory results are canonicalized by globally unique session UUID before list/search/time aggregation. If stale projections remain in multiple workspace indexes, the freshest observed file/index record wins so the same session is not counted twice.
...
- `turn_count`: canonical transcript turn count. `projected_item_count` is the separate UI/projected row count and must not be used for duration or turn analytics.
...
**Duration:** Computed at query time from stored interval primitives: provider-response spans are continuous intervals, while request, activity, summary, and turn-boundary timestamps are point observations. Intervals are merged to remove overlap. `idle_threshold_minutes` (settings-backed default, currently 10) splits the positive gaps between observations — gaps greater than the threshold are idle and excluded; gaps less than or equal are active and included. `active_duration_seconds` / `total_active_duration_seconds` reflect the requested threshold.
...
**Returns:** `session_id`, `session_name`, `workspace_name`, `total_turns`, `returned_turn_start`, `returned_turn_end`, `truncated`, `token_usage_summary`, and `turns`. Each returned turn includes `turn_index`, `started_at`, optional `request_text`, optional `tool_call_summary`, `run_ids`, `token_usage`, `entries`, `truncated`, and optional `entries_omitted`.

**Token accounting:** Claude-compatible provider usage rows persist `run_id` and `turn_id` for newly recorded runs and expose provider input/output totals plus estimated tool input/output totals. Older usage rows remain readable but are reported as unattributed. Codex currently exposes its separately persisted last-context and cumulative totals in `token_usage_summary`; those totals are not synthesized into per-turn rows. The two provider shapes stay explicit rather than being combined into a misleading cross-provider total.
...
- **Point observations split gaps**: a zero-duration request/activity/turn boundary is retained and can split one idle gap into two, which can change classification at a given threshold. Provider-response spans still contribute their measured continuous coverage.
