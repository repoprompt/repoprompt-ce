...
- Root package tools version: 6.2; package default and all targets outside the leaf-core language-mode phase remain in Swift 5 language mode.
...
| --- | --- | --- | --- |
| Dependency/grammar upgrade and broad parser-lock removal | `3f78ecc1025a7b85f632113f8bc751f688abccbf` | Swift 5 | Exact SwiftTreeSitter 0.10/runtime 0.25.10 and grammar revisions; scanner shim retained after clean-link proof. |
| Official-source cleanup | `ad6acd4541d3082aee548027ef56e34fcf3aaed9` | Swift 6 unchanged | PHP resolves from the official v0.24.2 source with updated cache identity; the wrapper URL migration remains blocked by Neon’s released dependency identity. |
| Generated-Xcode official-graph acquisition | `b398b57d7be6a5ac3efd721ed2b7c78beff9b402` | Swift 6 unchanged | Xcode package acquisition retains the authoritative graph and uses a process-local same-host HTTPS transport for public GitHub SSH submodule URLs; no dependency target, identity, revision, source, or language setting changes. |
| Official JavaScript/Python scanner cleanup | Blocked on an upstream release | Swift 6 unchanged | Official `v0.25.0` and default-branch manifests still omit both scanner objects in the root graph; a clean shim-free link failed on all ten external-scanner ABI symbols. |
| Exact grammar semantic-version requirements | `08912ab98924b9eb337ca6d569888016499557e8` | Swift 6 unchanged | Every released/buildable grammar uses an exact semantic version while resolved commits and CodeMap cache identity remain unchanged; Dart and Neon remain evidenced revision exceptions. |
| Highlighting query compatibility and C# coverage | `2b53f38a3ffaeb7908012b4b7a1e7f83fec375c6` | Swift 6 unchanged | All 14 app-owned highlighting queries compile against their registered grammars; C# completes the 14-language CodeMapCore fixture/golden matrix without changing the existing 13 outputs. |
| `RepoPromptRegexCore` | extraction `8434d881ee1a7eb44522892488c2f607b9d85d64`; leaf-core language-mode phase | Swift 6 | Production and owner-test targets compile with `-swift-version 6`; eight owner tests and Swift 5 app-consumer linkage pass. |
| `RepoPromptCodeMapCore` / owner tests | extraction `ff51f0d7acda0cca7b39a4041b356676559271fa`; leaf-core language-mode phase | Swift 6 | Production and owner-test targets compile with `-swift-version 6`; 16 owner tests, mixed-mode app tests, and both Swift 5 product builds pass. |

## CodeMap ownership extraction record
...
No new escape hatch or source annotation was added during the leaf-core language-mode phase. The four target logs and raw verbose invocation evidence contain zero diagnostics attributed to their source or owner-test paths.

## CodeMap ownership extraction evidence
...
## Leaf-core Swift 6 language-mode evidence
...
## Official-source cleanup evidence
...
- Target-local Swift 6 settings and behavior are unchanged. `TreeSitterScannerSupport` and generated-Xcode behavior remain untouched for the generated-Xcode acquisition and official scanner cleanup follow-ups.
...
## Official JavaScript/Python scanner cleanup blocker
...
## Independent review and hosted acquisition follow-up

- Independent range inspection and RepoPrompt Oracle review found no unresolved correctness, security, compatibility, dependency/cache identity, Xcode acquisition, Swift 6 preservation, or documentation defect in the official-source cleanup, generated-Xcode acquisition, or official scanner cleanup phases. Local lint, both products, full root/provider suites, guardrails, 26 generator contracts, clean Xcode validation, contribution `pr-ready`, and the authorized live CodeMap/Agent Mode smoke all passed.
...
## Exact grammar requirement evidence
...
## Query compatibility and C# coverage evidence
