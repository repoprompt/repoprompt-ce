...
| Portable runtime package source, tests, manifest, or lockfile | `make dev-portable-test`, one full `make dev-test`, and both root product builds; included in `pr-ready` for `Packages/RepoPromptPortableRuntime/**`. |
| `Sources/RepoPrompt/**` or `Packages/RepoPromptPortableRuntime/**` | `make dev-swift-build PRODUCT=RepoPrompt`; included in `pr-ready` for these paths. |
| `Sources/RepoPromptMCP/**` or `Packages/RepoPromptPortableRuntime/**` | `make dev-swift-build PRODUCT=repoprompt-mcp`; included in `pr-ready` for these paths. |
