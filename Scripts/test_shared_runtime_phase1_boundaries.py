...
def assert_frozen_phase0_artifacts() -> None:
    fixture_paths = subprocess.check_output(
        ["git", "ls-tree", "-r", "--name-only", BASELINE, PHASE0_PREFIX],
        cwd=ROOT,
        text=True,
    ).splitlines()
    ...
    if current_fixture_paths != fixture_paths:
        fail(
            ...
            f"{BASELINE}: baseline={fixture_paths}, current={current_fixture_paths}"
        )

    for relative in [*fixture_paths, *FROZEN_FILES]:
        current = (ROOT / relative).read_bytes()
        baseline = git_bytes(BASELINE, relative)
        if current != baseline:
            fail(f"Frozen Phase 0 artifact changed relative to {BASELINE}: {relative}")
