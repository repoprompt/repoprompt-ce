#!/usr/bin/env python3
"""Historical Phase 2 Slice 1 workspace-authority and Phase 0 boundary checks."""
...
def assert_phase0_fixtures_unchanged() -> None:
    baseline_paths = git_paths(PHASE0_ARTIFACT_BASELINE, PHASE0_ROOT)
    current_root = ROOT / PHASE0_ROOT
    ...
    for relative in baseline_paths:
        if (ROOT / relative).read_bytes() != git_bytes(PHASE0_ARTIFACT_BASELINE, relative):
            fail(
                "Frozen Phase 0 fixture changed relative to "
...
def main() -> int:
    ...
    phase0_baseline = git_paths(PHASE0_ARTIFACT_BASELINE, PHASE0_ROOT)
    if not phase0_baseline:
        fail(f"No Phase 0 fixtures found at {PHASE0_ARTIFACT_BASELINE}")
    assert_phase0_fixtures_unchanged()
