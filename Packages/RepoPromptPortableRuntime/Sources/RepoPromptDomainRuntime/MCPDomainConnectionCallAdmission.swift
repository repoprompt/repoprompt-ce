...
public struct MCPDomainConnectionCallAdmissionEntry: Sendable {
    public let limiters: MCPDomainConnectionCallLimiters
...
public final class MCPDomainConnectionCallAdmissionRegistry: @unchecked Sendable {
    ...
    public subscript(connectionID: UUID) -> MCPDomainConnectionCallLimiters? {
    ...
    @discardableResult
    public func removeValue(forKey connectionID: UUID) -> MCPDomainConnectionCallLimiters? {
        lock.withLock { entries.removeValue(forKey: connectionID)?.limiters }
    }

    public func snapshot() -> [UUID: MCPDomainConnectionCallLimiters] {
        lock.withLock { entries.mapValues(\.limiters) }
    }

    public func entry(for connectionID: UUID) -> MCPDomainConnectionCallAdmissionEntry? {
...
public enum MCPDomainConnectionCallLane: String, CaseIterable, Sendable {
    case ordinary
    case control
...
public struct MCPDomainConnectionCallLimiterWatchdogDiagnostics: Sendable {
    public let admittedCallCount: Int
}

public actor MCPDomainConnectionCallLimiters {
    public struct AdmissionRejected: Error {}
    ...
    #else
        public init(limit: Int, controlLimit: Int, smallReadLimit: Int, fileReadLimit: Int, gitReadLimit: Int, fileSearchLimit: Int) {
    ...
    public func hasInFlightCalls() -> Bool {
        admittedCallCount > 0
    }

    public func executionWatchdogDiagnostics() -> MCPDomainConnectionCallLimiterWatchdogDiagnostics {
        MCPDomainConnectionCallLimiterWatchdogDiagnostics(admittedCallCount: admittedCallCount)
    }

    public func admissionRetryReplacement() async -> MCPDomainConnectionCallLimiters? {
    ...
    public func markTentativeCloseRestored(by replacement: MCPDomainConnectionCallLimiters) {
    ...
    public func markTentativeCloseCommitted() {
        guard case .tentative = admissionCloseState else { return }
    ...
    #else
        public func closeIfIdle() async -> Bool {
            guard case .open = admissionCloseState, admittedCallCount == 0 else { return false }
            admissionCloseState = .tentative
            await closeLanes()
            return true
    ...
    public func waitUntilIdle(timeout: Duration) async -> [(MCPDomainConnectionCallLane, Bool)] {
    ...
    #if DEBUG
        public func limiterForTesting(_ lane: MCPDomainConnectionCallLane) -> MCPDomainAsyncLimiter {
            limiter(for: lane)
        }

        public func diagnosticsSnapshot() async -> MCPDomainConnectionCallLimiterDebugSnapshot {
        ...
        public func diagnosticsSnapshot(for lane: MCPDomainConnectionCallLane) async -> MCPDomainAsyncLimiter.DebugSnapshot {
            await limiter(for: lane).debugSnapshot()
        }

        public func admissionRetryWaiterCountForTesting() -> Int {
...
#if DEBUG
public struct MCPDomainConnectionCallLimiterDebugSnapshot: Equatable, Sendable {
    public let ordinary: MCPDomainAsyncLimiter.DebugSnapshot
    public let control: MCPDomainAsyncLimiter.DebugSnapshot
    public let smallRead: MCPDomainAsyncLimiter.DebugSnapshot
    public let fileRead: MCPDomainAsyncLimiter.DebugSnapshot
    public let gitRead: MCPDomainAsyncLimiter.DebugSnapshot
    public let fileSearch: MCPDomainAsyncLimiter.DebugSnapshot
    ...
    public var activePermitCount: Int {
    ...
    public var oldestWaiterAgeMilliseconds: UInt64? {
        [
    ...
    public var cancelledWaiterCount: Int {
...
public actor MCPDomainAsyncLimiter {
...
private let idleWaitSleep: @Sendable (Duration) async throws -> Void
    ...
    public struct DebugSnapshot: Equatable, Sendable {
        ...
        public let activePermitCount: Int
        public let waiterCount: Int
        public let inFlight: Int
        public let oldestWaiterAgeMilliseconds: UInt64?
        public let cancelledWaiterCount: Int
...
public func waitUntilIdle(timeout: Duration) async -> Bool {
    guard !Task.isCancelled else { return false }
    guard !isIdle else { return true }
...
private func resumeIdleWaitersIfNeeded() {
    ...
    public func setDebugStateObserver(
    ...
    public func setDebugQueuedPermitHandoffHandler(
