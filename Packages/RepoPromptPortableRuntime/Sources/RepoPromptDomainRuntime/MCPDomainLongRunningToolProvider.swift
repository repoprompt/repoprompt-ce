...
public enum DomainChildLaunchContext {
    @TaskLocal public static var current: DomainChildLaunchCarrier?
}

public enum DomainInteractionPresentationContext {
    @TaskLocal public static var requestID: UUID?
}

public struct DomainLongRunningInteractionAdapter: Sendable {
    public let isAvailable: @Sendable (DomainInteractionRequest) async -> Bool
    public let resolveDefaultTimeoutSeconds: @Sendable (DomainInteractionRequest) async throws -> TimeInterval
...
public struct MCPDomainLongRunningToolProvider: Sendable {
    public typealias PrepareChildLaunch = @Sendable (
        _ toolName: String,
