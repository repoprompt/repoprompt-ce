...
public enum DomainAgentWorktreeBindingStoreError: Error, LocalizedError, Equatable, Sendable {
    case futureDocument
    case wrongProfile
...
public actor DomainAgentWorktreeBindingStore {
    ...
    public func bindings(sessionID: UUID) -> [AgentSessionWorktreeBinding] {
        bindingsBySessionID[sessionID] ?? []
    }

    public func allBindings() -> [UUID: [AgentSessionWorktreeBinding]] {
        bindingsBySessionID
    }
    ...
    public func remove(sessionID: UUID, repositoryID: String? = nil) async throws -> UInt64 {
        try await mutate { records in
