...
public struct DomainAgentSessionRegistration: Codable, Equatable, Hashable, Sendable {
...
public struct DomainAgentSessionWaitCursor: Equatable, Hashable, Sendable {
    public let registration: DomainAgentSessionRegistration
...
public enum DomainAgentSessionDurableState: String, Codable, Sendable {
    case dormant
    case active
    case interrupted
    case terminal
...
public struct DomainAgentSessionDurableMetadata: Codable, Equatable, Sendable {
    public let sessionID: UUID
    public let owningRuntimeID: UUID
    public let owningRuntimeGeneration: UInt64
    public let registrationGeneration: UInt64
    public let lastEpochOrdinal: UInt64
    public let continuityGeneration: UInt64
    public let state: DomainAgentSessionDurableState
...
public struct DomainAgentSessionShutdownResult: Equatable, Sendable {
    public let cooperativeSessionIDs: [UUID]
    public let interruptedSessionIDs: [UUID]
}

public enum DomainAgentSessionResumeClaimResult: Equatable, Sendable {
...
public enum DomainAgentSessionPersistenceHealth: Equatable, Sendable {
    case ready
    case degradedReadOnly(reason: String)
}

public struct DomainAgentSessionStoreSnapshot: Equatable, Sendable {
    public let persistenceHealth: DomainAgentSessionPersistenceHealth
    public let retainedMetadataCount: Int
    public let omittedRetentionCount: UInt64
    public let pendingPersistenceCount: Int
}
...
public actor DomainAgentRunSessionStore {
    public typealias Registration = DomainAgentSessionRegistration
    public typealias WaitCursor = DomainAgentSessionWaitCursor
    ...
    public enum EpochBeginResult: Equatable, Sendable {
        case accepted(DomainAgentRunTurnEpoch)
        case stale(currentEpoch: DomainAgentRunTurnEpoch?)
        case rejected(reason: String)
    }

    public enum WaitDisposition: Equatable, Sendable {
    ...
    public struct NoteworthyWake: Equatable, Sendable {
        public let snapshot: DomainAgentRunSnapshot
        public let reason: WakeReason
        public let steeringMessage: String?
        public let steeringOriginRunID: UUID?
    ...
    public func restoredMetadata() -> [DomainAgentSessionDurableMetadata] {
        durableMetadata.values.sorted { $0.updatedAt > $1.updatedAt }
    }

    public func snapshot() -> DomainAgentSessionStoreSnapshot {
    ...
    public func claimResumableSession(sessionID: UUID) async -> DomainAgentSessionResumeClaimResult {
    ...
    public func register(sessionID: UUID) -> Registration {
    ...
    public func registerIfMissing(sessionID: UUID) -> Registration? {
    ...
    public func noteSnapshot(_ snapshot: DomainAgentRunSnapshot, cursor: WaitCursor) {
        ingestSnapshot(snapshot, cursor: cursor, wakeReason: nil)
    }

    public func noteSnapshotAndWakeWaiters(
    ...
    public func wakeCurrentWaiters(
        _ snapshot: DomainAgentRunSnapshot,
        cursor: WaitCursor,
        reason: WakeReason,
        steeringMessage: String? = nil,
    ...
    public func waitUntilInteresting(
        cursor: WaitCursor,
    ...
    public func snapshot(for cursor: WaitCursor) -> DomainAgentRunSnapshot? {
    ...
    public func snapshot(for registration: Registration) -> DomainAgentRunSnapshot? {
        guard let cursor = currentCursor(for: registration) else { return nil }
        return snapshot(for: cursor)
    }

    public func currentCursor(for registration: Registration) -> WaitCursor? {
    ...
    public func currentRegistration(for sessionID: UUID) -> Registration? {
        records[sessionID]?.registration
    }

    public func currentEpoch(for registration: Registration) -> DomainAgentRunTurnEpoch? {
        currentRecord(for: registration, operation: "current_epoch")?.currentEpoch
    }

    public func hasActiveRegistration(sessionID: UUID) -> Bool {
        records[sessionID] != nil
    }

    public func cleanup(registration: Registration) {
    ...
    @discardableResult
    public func installCancellationHandler(
        registration: Registration,
    ...
    public func removeCancellationHandler(registration: Registration) {
        guard currentRecord(for: registration, operation: "remove_cancellation") != nil else { return }
        cancellationHandlers.removeValue(forKey: registration.sessionID)
    }

    public func shutdown(deadline: Duration = .seconds(5)) async -> DomainAgentSessionShutdownResult {
        guard !isShuttingDown else {
    ...
    extension DomainAgentRunSessionStore {
        public func test_waiterCount(registration: Registration) -> Int {
        ...
        public func test_waiterIDs(registration: Registration) -> [UUID] {
        ...
        public func test_cancelWaiter(sessionID: UUID, waiterID: UUID) {
            cancelWaiter(sessionID: sessionID, waiterID: waiterID)
        }

        public func test_shutdownOwnedState() -> (recordCount: Int, cancellationHandlerCount: Int) {
            (records.count, cancellationHandlers.count)
        }

        public func test_setShutdownAfterDetach(
            _ hook: (@Sendable () async -> Void)?
        ) {
            testShutdownAfterDetach = hook
        }

        public func test_expire(cursor: WaitCursor) {
            expire(cursor: cursor)
        }

        public func test_setMetadataFlushBeforeCAS(
            _ hook: (@Sendable () async -> Void)?
        ) {
            testMetadataFlushBeforeCAS = hook
        }

        public func test_setTerminalCommitID(_ commitID: UUID, cursor: WaitCursor) {
