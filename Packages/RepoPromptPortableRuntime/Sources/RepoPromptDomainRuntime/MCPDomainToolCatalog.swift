...
public enum MCPGlobalToolName {
    public static let appSettings = "app_settings"
    public static let bindContext = "bind_context"
    public static let manageWorkspaces = "manage_workspaces"

    public static var orderedToolNames: [String] {
        MCPDomainToolCatalog.globalToolNames
    }
}

public enum MCPWindowToolName {
    public static let manageSelection = "manage_selection"
    public static let fileActions = "file_actions"
    public static let getCodeStructure = "get_code_structure"
    public static let getFileTree = "get_file_tree"
    public static let readFile = "read_file"
    public static let search = "file_search"
    public static let workspaceContext = "workspace_context"
    public static let prompt = "prompt"
    public static let applyEdits = "apply_edits"
    public static let oracleUtils = "oracle_utils"
    public static let askOracle = "ask_oracle"
    public static let oracleSend = "oracle_send"
    public static let oracleChatLog = "oracle_chat_log"
    public static let git = "git"
    public static let manageWorktree = "manage_worktree"
    public static let contextBuilder = "context_builder"
    public static let askUser = "ask_user"
    public static let agentExplore = "agent_explore"
    public static let agentRun = "agent_run"
    public static let agentManage = "agent_manage"
    public static let history = "history"
    public static let shareThoughts = "share_thoughts"
    public static let setStatus = "set_status"
    public static let waitForNextInstruction = "wait_for_next_user_instruction"
...
public enum MCPDomainToolScopeKind: String, CaseIterable, Sendable {
    case application
    case window
    case standalone
}

public enum MCPToolCapability: String, CaseIterable, Hashable, Sendable {
...
public enum MCPToolAdmissionClass: String, CaseIterable, Sendable {
    case exclusive
    case control
...
public enum MCPDomainToolOperationInput: Equatable, Sendable {
    case missing
    case value(String)
    case malformed
}

public struct MCPDomainToolOperationIdentity: Equatable, Hashable, Sendable {
    public static let unknownToolName = "unknown"
    public static let unknownOperation = "unknown"
    public static let callOperation = "call"
    ...
    public static let unknown = MCPDomainToolOperationIdentity(
        canonicalTool: unknownToolName,
        normalizedOperation: unknownOperation
    )
}

public enum MCPDomainToolResourceLimitScope: String, Sendable {
    case application
    case window
    case repository
}

public struct MCPDomainToolConfiguredLimits: Equatable, Sendable {
    public let connectionLane: Int
    public let resourceLease: Int?
    public let resourceScope: MCPDomainToolResourceLimitScope?
}
...
public struct MCPDomainToolCatalogEntry: Hashable, Sendable {
    public let name: String
    public let scope: MCPDomainToolScopeKind
    ...
    public func supports(registrationScope: MCPDomainToolRegistrationScope) -> Bool {
...
public enum MCPDomainToolCatalog {
    public static let entries: [MCPDomainToolCatalogEntry] = [
    ...
    public static let orderedToolNames = entries.map(\.name)
    public static let globalToolNames = entries.filter { $0.scope == .application }.map(\.name)
    public static let windowToolNames = entries.filter { $0.scope == .window }.map(\.name)
    public static let classifications = Dictionary(uniqueKeysWithValues: entries.map { ($0.name, $0.admissionClass) })

    private static let entriesByName = Dictionary(uniqueKeysWithValues: entries.map { ($0.name, $0) })

    public static func entry(named toolName: String) -> MCPDomainToolCatalogEntry? {
    ...
    public static func admissionClass(for toolName: String) -> MCPToolAdmissionClass? {
        entry(named: toolName)?.admissionClass
    }

    public static func operationArgumentKey(for toolName: String) -> String? {
        entry(named: toolName)?.operationPolicy?.argumentKey
    }

    public static func operationIdentity(
        for toolName: String,
    ...
    public static func configuredLimits(for toolName: String) -> MCPDomainToolConfiguredLimits? {
