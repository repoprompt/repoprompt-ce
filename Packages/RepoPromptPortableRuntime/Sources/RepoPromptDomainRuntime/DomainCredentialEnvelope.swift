...
public struct DomainCredentialScope: Codable, Hashable, Sendable {
    public let providerIdentifier: String
    public let runID: UUID
    public let principalID: UUID
    public let purpose: String
    public let accountIdentifierDigest: String?
...
public struct DomainCredentialEnvelopeDescriptor: Hashable, Sendable {
    public let envelopeID: UUID
    public let runtimeID: UUID
    public let runtimeGeneration: UInt64
    public let scope: DomainCredentialScope
...
public enum DomainCredentialPayloadError: Error, Equatable, Sendable {
...
public final class DomainCredentialPayload: @unchecked Sendable, CustomStringConvertible {
    ...
    public func withConsumedBytes<Result>(
    ...
    #if DEBUG
        public func test_ownedStorageBytes() -> [UInt8] {
            storage.testSnapshot()
        }

        public func test_isOwnedStorageZeroed() -> Bool {
            storage.testIsZeroed()
        }

        public func test_expiresAt() -> ContinuousClock.Instant {
            expiresAt
        }
    #endif
}

public enum DomainCredentialEnvelopeError: Error, Equatable, Sendable {
    case unavailable
...
public actor DomainCredentialEnvelopeStore {
    public static let maximumPayloadBytes = 64 * 1024
    public static let maximumOutstandingEnvelopeCount = 256
    public static let maximumTombstoneCount = 256
    ...
    public init(identity: DomainRuntimeIdentity) {
    ...
    public func revoke(_ envelopeID: UUID) {
    ...
    public func shutdown() {
    ...
    #if DEBUG
        public func test_ownedStorageBytes(envelopeID: UUID) -> [UInt8]? {
            records[envelopeID]?.storage?.testSnapshot()
        }

        public func test_isOwnedStorageZeroed(envelopeID: UUID) -> Bool? {
            guard let record = records[envelopeID] else { return nil }
            return record.storage?.testIsZeroed() ?? (record.state != .active)
        }

        public func test_recordCount() -> Int {
            records.count
        }

        public func test_activeEnvelopeCount() -> Int {
            activeEnvelopeCount
        }

        public func test_tombstoneCount() -> Int {
            terminalOrder.count
        }

        public func test_terminalStorageCount() -> Int {
        ...
        public func test_expiryTaskCount() -> Int {
            expiryTasks.count
        }
    #endif
}

public struct DomainChildLaunchCarrier: Sendable {
    public static let endpointEnvironmentKey = "REPOPROMPT_MCP_PRIVATE_ENDPOINT"
    public static let launchTokenEnvironmentKey = "REPOPROMPT_MCP_LAUNCH_TOKEN"
    public static let credentialEnvelopeEnvironmentKey = "REPOPROMPT_MCP_CREDENTIAL_ENVELOPE"
    public static let clientPrincipalEnvironmentKey = "REPOPROMPT_MCP_CLIENT_PRINCIPAL"
    public static let providerIdentifierEnvironmentKey = "REPOPROMPT_MCP_PROVIDER_IDENTIFIER"
    public static let runIDEnvironmentKey = "REPOPROMPT_MCP_RUN_ID"

    public let runID: UUID
    public let launchTokenID: UUID
    public let credentialEnvelope: DomainCredentialEnvelopeDescriptor?
...
public struct DomainPrivateChildLaunchHarness: Sendable {
    public typealias IssueLaunchToken = @Sendable (
