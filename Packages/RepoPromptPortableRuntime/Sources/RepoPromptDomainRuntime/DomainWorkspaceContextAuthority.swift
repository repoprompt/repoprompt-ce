...
public enum DomainExternalReloadActivity: Equatable, Sendable {
    case changed
    case unchanged
    case recoveryPending
}

public struct DomainWorkspaceStore: Sendable {
    ...
    public func snapshot() async -> DomainWorkspaceCatalogSnapshot {
        await authority.readySnapshot()
    }

    public func subscribe() async -> DomainWorkspaceSnapshotSubscription {
        await authority.readySubscription()
    }

    public func execute(_ command: DomainWorkspaceCommandEnvelope) async -> DomainCommandOutcome {
        await authority.execute(command)
    }

    @discardableResult
    public func reloadExternalChanges() async -> DomainExternalReloadActivity {
        await authority.reloadExternalChanges()
    }

    #if DEBUG
        public func testSetBeforeExternalReconciliation(
    ...
    public func registerReadDocument(_ document: DomainWorkspaceDocument) async -> DomainWorkspaceSnapshot {
        await authority.registerReadDocument(document)
    }

    public func workspaceSnapshot(_ workspaceID: UUID) async -> DomainWorkspaceSnapshot? {
    ...
    public func canonicalWorkspaceSnapshot(_ workspaceID: UUID) async -> DomainWorkspaceSnapshot? {
        await authority.canonicalWorkspaceSnapshot(workspaceID)
    }
}

public struct DomainContextStore: Sendable {
    private let authority: DomainWorkspaceContextAuthority
    ...
    public func snapshot(_ identity: DomainContextIdentity) async -> DomainContextSnapshot? {
