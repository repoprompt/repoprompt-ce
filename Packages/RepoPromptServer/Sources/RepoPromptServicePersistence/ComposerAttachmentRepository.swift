...
public extension SQLiteServiceStore {
    ...
    func authorityStore_composerAttachments(actorID: String? = nil, projectID: UUID? = nil, lifecycle: ComposerAttachmentLifecycle? = nil) async throws -> [StoredComposerAttachment] {
        ...
        return try await connection.query("SELECT * FROM composer_attachments\(suffix) ORDER BY created_at", bindings).map(decodeComposerAttachment)
    }

    func authorityStore_deleteComposerAttachment(attachmentID: UUID) async throws {
        _ = try await connection.query("DELETE FROM composer_attachments WHERE attachment_id=?", [.text(attachmentID.uuidString)])
    }
    ...
    func persistAcceptedAttachmentManifest(turnID: UUID, sessionID: UUID, manifest: AgentTurnContentWire, totalBytes: Int, at date: Date) async throws {
        ...
        _ = try await connection.query("INSERT INTO accepted_attachment_manifests(turn_id,session_id,schema_version,manifest_json,total_bytes,created_at) VALUES(?,?,1,?,?,?) ON CONFLICT(turn_id) DO NOTHING", [.text(turnID.uuidString), .text(sessionID.uuidString), .text(json), .integer(totalBytes), .float(date.timeIntervalSince1970)])
    }
