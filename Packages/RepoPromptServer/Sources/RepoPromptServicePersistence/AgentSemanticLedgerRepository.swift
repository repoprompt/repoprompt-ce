...
public extension SQLiteServiceStore {
    ...
    func authorityStore_agentSubmission(actorID: String, targetKey: String, operation: String, publicKey: String) async throws -> AgentSubmissionRecord? {
        guard let row = try await connection.query("SELECT * FROM agent_submissions WHERE actor_id=? AND target_key=? AND operation=? AND public_key=?", [.text(actorID), .text(targetKey), .text(operation), .text(publicKey)]).first else { return nil }
        return try decodeSubmission(row)
    }

    func authorityStore_agentSubmission(submissionID: UUID) async throws -> AgentSubmissionRecord? {
        guard let row = try await connection.query("SELECT * FROM agent_submissions WHERE submission_id=?", [.text(submissionID.uuidString)]).first else { return nil }
        return try decodeSubmission(row)
    }
    ...
    func authorityStore_agentSubmissions(state: AgentSubmissionState? = nil, dispatchState: String? = nil, limit: Int = 500) async throws -> [AgentSubmissionRecord] {
        ...
        case let (.some(state), .some(dispatchState)):
            rows = try await connection.query("SELECT * FROM agent_submissions WHERE state=? AND dispatch_state=? ORDER BY created_at LIMIT ?", [.text(state.rawValue), .text(dispatchState), .integer(bounded)])
        case let (.some(state), nil):
            rows = try await connection.query("SELECT * FROM agent_submissions WHERE state=? ORDER BY created_at LIMIT ?", [.text(state.rawValue), .integer(bounded)])
        case let (nil, .some(dispatchState)):
            rows = try await connection.query("SELECT * FROM agent_submissions WHERE dispatch_state=? ORDER BY created_at LIMIT ?", [.text(dispatchState), .integer(bounded)])
        case (nil, nil):
            rows = try await connection.query("SELECT * FROM agent_submissions ORDER BY created_at LIMIT ?", [.integer(bounded)])
        }
    ...
    func authorityStore_rejectAgentSubmission(submissionID: UUID, code: String, at date: Date) async throws {
        _ = try await connection.query("UPDATE agent_submissions SET state='rejected',rejection_code=?,updated_at=? WHERE submission_id=? AND state='preparing'", [.text(code), .float(date.timeIntervalSince1970), .text(submissionID.uuidString)])
    }

    func authorityStore_markSubmissionDispatch(submissionID: UUID, state: String, at date: Date) async throws {
        _ = try await connection.query("UPDATE agent_submissions SET dispatch_state=?,updated_at=? WHERE submission_id=? AND state='accepted'", [.text(state), .float(date.timeIntervalSince1970), .text(submissionID.uuidString)])
    }
    ...
    func authorityStore_commitAgentSubmission(
        ...
        try await transaction {
            ...
            _ = try await connection.query("INSERT INTO effective_turn_configurations(turn_id,session_id,request_anchor_id,schema_version,configuration_json,accepted_at) VALUES(?,?,?,?,?,?)", [.text(turn.identity.turnID.uuidString), .text(turn.sessionID.uuidString), .text(turn.identity.requestAnchorID.uuidString), .integer(turn.effectiveConfiguration.schemaVersion), .text(configurationJSON), .float(turn.acceptedAt.timeIntervalSince1970)])
            _ = try await connection.query("INSERT INTO session_next_turn_defaults(session_id,schema_version,revision,configuration_json,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(session_id) DO UPDATE SET schema_version=excluded.schema_version,revision=excluded.revision,configuration_json=excluded.configuration_json,updated_at=excluded.updated_at", [.text(nextDefaults.sessionID.uuidString), .integer(nextDefaults.schemaVersion), .integer(Int(nextDefaults.revision)), .text(try encodedText(nextDefaults.configuration)), .float(nextDefaults.updatedAt.timeIntervalSince1970)])
            ...
            let leasedRows = try await connection.query("SELECT attachment_id,byte_size FROM composer_attachments WHERE lease_submission_id=? AND lifecycle='staged'", [.text(record.submissionID.uuidString)])
    ...
    func authorityStore_nextTurnDefaults(sessionID: UUID) async throws -> SessionNextTurnDefaultsRecord? {
        guard let row = try await connection.query("SELECT schema_version,revision,configuration_json,updated_at FROM session_next_turn_defaults WHERE session_id=?", [.text(sessionID.uuidString)]).first,
    ...
    func authorityStore_runPresentation(sessionID: UUID) async throws -> RunPresentationSnapshot? {
        guard let text = try await connection.query("SELECT snapshot_json FROM run_presentations WHERE session_id=? ORDER BY generation DESC LIMIT 1", [.text(sessionID.uuidString)]).first?.column("snapshot_json")?.string else { return nil }
    ...
    func authorityStore_semanticTurn(runID: UUID) async throws -> SemanticTurnRecord? {
        guard let row = try await connection.query("SELECT * FROM semantic_turns WHERE run_id=? ORDER BY accepted_at DESC LIMIT 1", [.text(runID.uuidString)]).first else { return nil }
    ...
    func authorityStore_settleSemanticTurn(runID: UUID, terminalState: String, at date: Date) async throws {
        _ = try await connection.query("UPDATE semantic_turns SET terminal_state=COALESCE(terminal_state,?),settled_at=COALESCE(settled_at,?) WHERE run_id=?", [.text(String(terminalState.prefix(128))), .float(date.timeIntervalSince1970), .text(runID.uuidString)])
    }

    func authorityStore_latestSemanticSequence(sessionID: UUID) async throws -> Int64 {
        Int64(try await connection.query("SELECT MAX(last_sequence) AS sequence FROM semantic_turns WHERE session_id=?", [.text(sessionID.uuidString)]).first?.column("sequence")?.integer ?? 0)
    }
    ...
    func authorityStore_semanticActivity(activityID: UUID) async throws -> SemanticActivityRecord? {
        guard let row = try await connection.query("SELECT * FROM semantic_activities WHERE activity_id=?", [.text(activityID.uuidString)]).first else { return nil }
    ...
    func authorityStore_semanticActivities(turnID: UUID) async throws -> [SemanticActivityRecord] {
        try await connection.query("SELECT * FROM semantic_activities WHERE turn_id=? ORDER BY canonical_sequence,activity_id", [.text(turnID.uuidString)]).map { try decodeSemanticActivity($0) }
    }
    ...
    private func semanticTool(executionID: String) async throws -> SemanticToolRecord? {
        guard let row = try await connection.query("SELECT * FROM semantic_tools WHERE execution_id=?", [.text(executionID)]).first else { return nil }
