...
final class DirectHeadlessProcessTests: XCTestCase {
    ...
    func testNoAppHeadlessProcessListsCanonicalPolicySurfaceAndDrainsOnEOF() throws {
        let executable = try Self.helperExecutableURL()
        ...
        process.arguments = ["--launcher-contract-version", "1"]
    ...
    private static func repositoryRoot(startingAt directory: URL) throws -> URL {
        ...
        throw XCTSkip("headless process policy test requires a Git worktree")
    }
    ...
    private static func helperExecutableURL() throws -> URL {
        ...
        let fallback = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent(".build/debug/repoprompt-mcp-headless-runtime")
        guard FileManager.default.isExecutableFile(atPath: fallback.path) else {
            throw XCTSkip("private headless helper product is not built")
        }
        return fallback
    }
