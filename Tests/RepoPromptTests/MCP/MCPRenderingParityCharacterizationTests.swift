...
final class MCPRenderingParityCharacterizationTests: XCTestCase {
    ...
    func testCodeStructureDTOFreezesOrderingFullPathDedupAndOmissionCounts() async throws {
        ...
        let full = try await window.mcpServer.buildCodeStructureDTO(
        ...
        let limited = try await window.mcpServer.buildCodeStructureDTO(
            fromRecords: unsortedWithDuplicate,
            maxResults: 1,
            includeUnmappedPaths: true
        )
    ...
    func testWorkspaceContextIncludeSetsFreezeEmptyComponentsAndAppEnvelope() async throws {
        ...
        let empty = try await window.mcpServer.buildTabWorkspaceContext(
            context: context,
            include: [],
            display: .relative,
        ...
        let promptOnly = try await window.mcpServer.buildTabWorkspaceContext(
            context: context,
            include: ["prompt"],
            display: .relative,
            copyPresetOverride: override
        ...
        let filesOnly = try await window.mcpServer.buildTabWorkspaceContext(
            context: context,
            include: ["files"],
            display: .relative,
            copyPresetOverride: override
        ...
        let selectionAndCode = try await window.mcpServer.buildTabWorkspaceContext(
            context: context,
        ...
        let tokensOnly = try await window.mcpServer.buildTabWorkspaceContext(
            context: context,
            include: ["tokens"],
            display: .relative,
            copyPresetOverride: override
