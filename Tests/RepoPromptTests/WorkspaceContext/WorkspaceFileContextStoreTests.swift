...
final class WorkspaceFileContextStoreTests: XCTestCase {
    ...
    func testProjectionCaptureCoverageScopesCatalogRecordsAndCodemaps() async throws {
        let root = try makeTemporaryRoot(name: "ProjectionCaptureCoverage")
        ...
        let unrelatedURL = root.appendingPathComponent("Unrelated.swift")
        let selectedFolderURL = root.appendingPathComponent("Sources", isDirectory: true)
        try FileManager.default.createDirectory(at: selectedFolderURL, withIntermediateDirectories: true)
        let folderFirstURL = selectedFolderURL.appendingPathComponent("First.swift")
        let folderSecondURL = selectedFolderURL.appendingPathComponent("Second.swift")
        for url in [
            selectedURL,
            autoURL,
            sliceURL,
            ...
            try write("struct \(url.deletingPathExtension().lastPathComponent) {}", to: url)
        }
        ...
        await store.applyObservedCodemapResults([
            WorkspaceObservedCodemapResult(
                ...
                fileAPI: makeFileAPI(path: autoURL.path, symbolName: "AutoSymbol")
            ),
            WorkspaceObservedCodemapResult(
                fullPath: completeOnlyURL.path,
                modificationDate: Date(timeIntervalSince1970: 2),
                fileAPI: makeFileAPI(path: completeOnlyURL.path, symbolName: "CompleteSymbol")
            )
        ])
        let ranges = [LineRange(start: 1, end: 1)]
        let selection = StoredSelection(
            selectedPaths: [selectedURL.path, selectedFolderURL.path],
            autoCodemapPaths: [autoURL.path],
            slices: [sliceURL.path: ranges],
            codemapAutoEnabled: true
        )
        let noneTree = WorkspaceFileTreeSnapshotRequest(
            mode: .none,
            filePathDisplay: .relative,
            onlyIncludeRootsWithSelectedFiles: false,
            includeLegend: false,
        ...
        let referenced = try await store.captureWorkspaceFileContext(
            selection: selection,
            fileTreeRequest: noneTree,
            coverage: .projection(codemapCoverage: .referenced)
        )
        XCTAssertEqual(referenced.coverage, .projection(codemapCoverage: .referenced))
        XCTAssertEqual(referenced.catalog.roots.count, 1)
        XCTAssertTrue(referenced.catalog.files.isEmpty)
        XCTAssertTrue(referenced.catalog.entries.isEmpty)
        XCTAssertEqual(referenced.catalog.diagnostics.fileCount, 0)
        XCTAssertEqual(referenced.catalog.diagnostics.folderCount, 0)
        XCTAssertTrue(referenced.fileTree.roots.isEmpty)
        XCTAssertEqual(referenced.materializedFolders.map(\.standardizedRelativePath), ["Sources"])
        XCTAssertEqual(
            referenced.materializedFiles.map(\.standardizedRelativePath),
            ["Auto.swift", "Selected.swift", "Slice.swift", "Sources/First.swift", "Sources/Second.swift"]
        )
        XCTAssertEqual(referenced.codemapSnapshots.map(\.relativePath), ["Auto.swift"])

        let allAvailable = try await store.captureWorkspaceFileContext(
            selection: selection,
            fileTreeRequest: noneTree,
            coverage: .projection(codemapCoverage: .allAvailable)
        )
        XCTAssertEqual(allAvailable.coverage, .projection(codemapCoverage: .allAvailable))
        XCTAssertEqual(
            allAvailable.materializedFiles.map(\.standardizedRelativePath),
            [
                "Auto.swift",
                "CompleteOnly.swift",
                "Selected.swift",
        ...
        XCTAssertEqual(
            allAvailable.codemapSnapshots.map(\.relativePath),
            ["Auto.swift", "CompleteOnly.swift"]
        )

        let treeFallback = try await store.captureWorkspaceFileContext(
            selection: selection,
            fileTreeRequest: WorkspaceFileTreeSnapshotRequest(
                mode: .selected,
                filePathDisplay: .relative,
        ...
        XCTAssertEqual(treeFallback.coverage, .complete)
        XCTAssertEqual(treeFallback.catalog.files.count, 7)
        XCTAssertEqual(treeFallback.materializedFiles.count, 7)
    }
