...
final class DevinOMPModelSelectionTests: XCTestCase {
    ...
    /// Devin is interactive-only and must not leak into the non-agent chat/Oracle surface.
    func testDevinIsAbsentFromTheChatProviderSurface() {
        XCTAssertNil(AppSettingsMCPService.test_aiProviderType(for: .devin))
        XCTAssertEqual(AppSettingsMCPService.test_aiProviderType(for: .omp), .omp)
    }

    /// OMP authenticates through its own CLI, so RepoPrompt must not claim a keychain
    /// account for it.
    func testOMPHasNoSecureStorageAccount() {
