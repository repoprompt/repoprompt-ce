...
final class WorkspaceFolderOpenResolverTests: XCTestCase {
    ...
    @MainActor
    func testMenuOrderingUsesLibraryRecencyIndependentlyOfModificationTime() async {
        var fixtures = [
        ...
        for index in fixtures.indices {
            fixtures[index].isSavedWorkspace = true
        }
        fixtures[0].lastUsed = Date(timeIntervalSince1970: 30)
