...
final class HeadlessStateTransactionTests: XCTestCase {
    func testSelectionMutationRejectsStaleActiveWorkspace() async throws {
        let directory = HeadlessTestTemporaryDirectory.baseURL
            .appendingPathComponent("RepoPromptHeadlessSelectionTransactionTests-\(UUID().uuidString)", isDirectory: true)
        ...
        _ = try store.update { $0.allowedRoots = [root] }
        let host = HeadlessHost(configurationStore: store)
        let first = try await host.createWorkspace(name: "First", rootTokens: [root.name])
        _ = try await host.createWorkspace(name: "Second", rootTokens: [root.name])

        do {
            _ = try await host.updateSelection(workspaceID: first.id) { selection in
                selection = [HeadlessSelectionEntry(rootID: root.id, relativePath: "file.txt", mode: .full)]
            }
            XCTFail("Expected stale active-workspace mutation to fail")
        } catch {
            // Expected: the second workspace became active before this commit.
        }
        XCTAssertEqual(try HeadlessWorkspaceStore(paths: paths).loadWorkspace(id: first.id)?.selection, [])
    }
