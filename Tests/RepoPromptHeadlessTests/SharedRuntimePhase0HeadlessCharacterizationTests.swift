...
final class SharedRuntimePhase0HeadlessCharacterizationTests: XCTestCase {
    ...
    func testHeadlessPhase0CharacterizationSnapshot() async throws {
        ...
        let replacements = [
            fixture.stateDirectory.path: "$STATE",
            fixture.stateDirectory.resolvingSymlinksInPath().standardizedFileURL.path: "$STATE",
            fixture.rootDirectory.path: "$ROOT",
            fixture.rootDirectory.resolvingSymlinksInPath().standardizedFileURL.path: "$ROOT"
        ]
        normalizePaths(in: &initialize, replacements: replacements)
        if var serverInfo = initialize["serverInfo"] as? [String: Any] {
            // Executable release metadata is not part of the frozen Phase 0 behavior contract.
            serverInfo["version"] = "1.0.6"
            initialize["serverInfo"] = serverInfo
        }
    ...
    func testDangerousToolsStayHiddenAndFailClosedWhenAllPermissionsAreEnabled() async throws {
        ...
        for (toolName, capability) in HeadlessToolRegistry.blockedCapabilities {
            let response = try await registry.call(name: toolName, arguments: [:])
    ...
    func testHeadlessV1WorkspaceFixtureLoadsWithoutRewrite() throws {
        ...
        let source = repositoryRoot
            .appendingPathComponent("Tests/RepoPromptHeadlessTests/Fixtures/Phase0/Headless/ProfileV1", isDirectory: true)
    ...
    private static func normalizedValue(_ value: Any, replacements: [String: String]) -> Any {
        if let string = value as? String {
            ...
            return replacements.sorted { $0.key.count > $1.key.count }.reduce(string) { result, replacement in
    ...
    private func assertOrRecord(_ snapshot: [String: Any], fixtureName: String) throws {
        let fixtureURL = try HeadlessTestRepoRoot.url()
            .appendingPathComponent("Tests/RepoPromptHeadlessTests/Fixtures/Phase0/Headless", isDirectory: true)
