...
final class HeadlessStateFileSecurityTests: XCTestCase {
    ...
    func testSymlinkedStateParentIsRejectedWithoutCreatingOutsideState() throws {
        let directory = try makeTemporaryDirectory()
        let outsideParent = directory.appendingPathComponent("OutsideParent", isDirectory: true)
        try FileManager.default.createDirectory(at: outsideParent, withIntermediateDirectories: true)
        let linkedParent = directory.appendingPathComponent("LinkedParent", isDirectory: true)
        try FileManager.default.createSymbolicLink(
            atPath: linkedParent.path,
            withDestinationPath: outsideParent.path
        )
        let stateRoot = linkedParent.appendingPathComponent("v1", isDirectory: true)

        XCTAssertThrowsError(try HeadlessStatePaths(rootDirectory: stateRoot).ensureBaseDirectories())
        XCTAssertFalse(FileManager.default.fileExists(atPath: outsideParent.appendingPathComponent("v1").path))
    }
