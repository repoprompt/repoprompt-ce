import Darwin
@testable import RepoPromptPOSIXSupport
import XCTest

final class POSIXDescriptorSupportTests: XCTestCase {
    func testSetCloseOnExecPreservesExistingDescriptorFlags() throws {
    ...
    func testInvalidDescriptorProducesTypedError() {
        XCTAssertThrowsError(try POSIXDescriptorSupport.setCloseOnExec(-1)) { error in
            XCTAssertEqual(
                error as? POSIXDescriptorConfigurationError,
                .invalidFileDescriptor(fd: -1)
            )
        }
    }

    func testNegativeSocketShutdownIsANoOp() {
