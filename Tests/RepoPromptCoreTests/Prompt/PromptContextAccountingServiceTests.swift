...
final class PromptContextAccountingServiceTests: XCTestCase {
    ...
    func testCaptureBackedAccountingUsesCoreProjectionPlanAndPreservesProvenance() async throws {
        let root = try temporaryRoots.makeRoot(suiteName: "CaptureBackedAccounting")
        let selected = root.appendingPathComponent("Selected.swift")
        let auto = root.appendingPathComponent("Auto.swift")
        try FileSystemTestSupport.write("struct Selected {}", to: selected)
        try FileSystemTestSupport.write("struct Auto {}", to: auto)
        ...
        await store.applyObservedCodemapResults([
            WorkspaceObservedCodemapResult(
                ...
                fileAPI: makeFileAPI(path: auto.path, symbol: "autoSymbol")
            )
        ])
        let selection = StoredSelection(
        ...
        let result = try await PromptContextAccountingService().calculatePromptStats(
            request: request,
            store: store,
            capture: capture
        )

        XCTAssertEqual(result.captureProvenance, capture.provenance)
        XCTAssertEqual(result.resolvedEntries.map(\.file.standardizedRelativePath), [
            "Selected.swift",
            "Auto.swift"
        ])
        XCTAssertEqual(result.resolvedEntries.map(\.mode), [.fullFile, .codemap])
        XCTAssertEqual(result.resolvedEntries.map(\.loadedContent), ["struct Selected {}", nil])
        XCTAssertGreaterThan(result.tokenResult.totalTokenCount, 0)
        XCTAssertTrue(capture.fileTree.roots.isEmpty)
    }

    func testCaptureBackedAccountingRejectsDifferentSelection() async throws {
        let root = try temporaryRoots.makeRoot(suiteName: "CaptureSelectionMismatch")
        let file = root.appendingPathComponent("A.swift")
        try FileSystemTestSupport.write("struct A {}", to: file)

        let store = WorkspaceFileContextStore()
        _ = try await store.loadRoot(path: root.path)
        let captureSelection = StoredSelection(selectedPaths: [file.path])
        let capture = try await store.captureWorkspaceFileContext(
        ...
        do {
            _ = try await PromptContextAccountingService().calculatePromptStats(
                request: PromptContextAccountingRequest(selection: StoredSelection()),
                store: store,
                capture: capture
            )
            XCTFail("Expected capture selection mismatch")
        } catch let error as PromptContextAccountingError {
            XCTAssertEqual(error, .captureSelectionMismatch)
        }
    }
