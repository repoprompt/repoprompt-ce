...
final class SelectionSlicePersistenceAndRebaseTests: XCTestCase {
    ...
    func testPartitionStoreCASConflictPreservesNewerPartition() async throws {
        ...
        func testPartitionStoreCancellationAfterAtomicWriteStillSucceedsAndNotifies() async throws {
            ...
            let saveEvents = LockedPartitionStoreSaveEvents()
            let store = PartitionStore(baseURL: baseURL) { event in
                saveEvents.append(event)
                notified.fulfill()
            }
            ...
            XCTAssertEqual(saveEvents.snapshot().map(\.sourceID), [sourceID])
    ...
    func testSliceRebaseTransformsOverlapMatrixWithStableAffinity() {
        ...
        let updated = try? XCTUnwrap(StoredSelectionSliceMutation.rebasedSelection(
            selection,
            for: editedPath,
...
#if DEBUG
    private final class LockedPartitionStoreSaveEvents: @unchecked Sendable {
        private let lock = NSLock()
        private var events: [PartitionStoreSaveEvent] = []

        func append(_ event: PartitionStoreSaveEvent) {
            lock.lock()
            events.append(event)
            lock.unlock()
        }

        func snapshot() -> [PartitionStoreSaveEvent] {
            lock.lock()
            defer { lock.unlock() }
            return events
        }
